(function(){
  let healthRequest=null,healthCache=null,healthCachedAt=0,portonRequest=null,portonCache=null,portonCachedAt=0;
  ...
  function moneyValue(o){if(window.__vlaFinancialFailClosed)return null;try{return typeof calc==='function'?calc(o):null}catch{return null}}
  function addShell(){
  ...
  function enhanceDashboard(){
    ...
    document.getElementById('vla-health-refresh').onclick=()=>premiumHealth(true);
  ...
  async function premiumHealth(force=false){
    const host=document.getElementById('vla-health-mini');if(!host||typeof adminFetch!=='function')return;
    try{if(!force&&healthCache&&Date.now()-healthCachedAt<30000){renderHealthMini(host,healthCache);return}if(!healthRequest)healthRequest=adminFetch('/.netlify/functions/system-health-advanced').then(data=>{healthCache=data;healthCachedAt=Date.now();return data}).finally(()=>{healthRequest=null});renderHealthMini(host,await healthRequest)}catch(e){host.innerHTML=`<div class="vla-health-row"><span class="vla-dot" style="background:#ef4444"></span><span>${esc(e.message)}</span><span class="vla-pill bad">Error</span></div>`}
  }
  function renderHealthMini(host,d){const checks=d.checks||[],wanted=['Airtable','BCV','SMTP','Portón','respaldo','Netlify'],chosen=[];wanted.forEach(w=>{const c=checks.find(x=>String(x.name||'').toLowerCase().includes(w.toLowerCase()));if(c&&!chosen.includes(c))chosen.push(c)});checks.forEach(c=>{if(chosen.length<6&&!chosen.includes(c))chosen.push(c)});host.innerHTML=chosen.slice(0,6).map(c=>`<div class="vla-health-row"><span class="vla-dot" style="background:${c.severity==='ok'?'#22c55e':c.severity==='warning'?'#f59e0b':'#ef4444'}"></span><span>${esc(c.name)}</span><span class="vla-pill ${c.severity==='ok'?'ok':c.severity==='warning'?'warn':'bad'}">${c.severity==='ok'?'Operativo':c.severity==='warning'?'Revisar':'Error'}</span></div>`).join('')}
  async function premiumPorton(){
    try{if(portonCache&&Date.now()-portonCachedAt<30000)return renderPorton(portonCache);if(!portonRequest)portonRequest=adminFetch('/.netlify/functions/access-mode').then(data=>{portonCache=data;portonCachedAt=Date.now();return data}).finally(()=>{portonRequest=null});renderPorton(await portonRequest)}catch{document.getElementById('vla-porton-value').textContent='N/D';document.getElementById('vla-porton-meta').textContent='No disponible'}
  }
  function renderPorton(data){const automation=data.automation||{},base=data.mode||'—',active=automation.masterEnabled===true&&automation.accessEnabled===true&&automation.closeEnabled===true&&automation.onlyExpiredDebt===true&&Number(automation.restrictionDay)===1;document.getElementById('vla-porton-value').textContent=active?'Activo':base==='Manual'?'Manual':'Automático';document.getElementById('vla-porton-meta').textContent=active?'Día 1 · solo deuda anterior; cuota nueva excluida':automation.masterEnabled===true?'Piloto activo, motor de acceso pausado':'Piloto inteligente pausado'}
  function premiumRender(){
    if(!document.getElementById('vla-dashboard-panels')||typeof owners==='undefined')return;
    if(window.__vlaFinancialFailClosed){['vla-usd-label','vla-bs-label','vla-legend-usd','vla-legend-bs','vla-legend-total'].forEach(id=>{const node=document.getElementById(id);if(node)node.textContent='N/D'});document.getElementById('vla-validation').innerHTML='<span class="vla-pill bad" style="width:100%;justify-content:center;border-radius:9px">Información financiera temporalmente no disponible</span>';return}
    let total=0,u=0,b=0,diffs=0,fav=0;(owners||[]).forEach(o=>{const c=moneyValue(o);if(c.total>.01){total+=c.total;u+=Math.max(0,c.debtUsd||0);b+=Math.max(0,c.debtBs||0)}if(c.netTotal<-.01)fav++;if(Math.abs(c.diff||0)>.01)diffs++});
    ...
    document.getElementById('vla-validation').innerHTML=typeof transitionMode==='function'&&transitionMode()?`<span class="vla-pill warn" style="width:100%;justify-content:center;border-radius:9px">Datos históricos presentes · autoridad canónica v7 · ${fav} saldo(s) a favor</span>`:`<span class="vla-pill ok" style="width:100%;justify-content:center;border-radius:9px">Contrato financiero canónico v7</span>`;
  ...
  function wrapRender(){
    if(typeof renderAll==='function'&&!renderAll.__vlaPremium){const original=renderAll;const wrapped=function(){const r=original.apply(this,arguments);setTimeout(premiumRender,0);return r};wrapped.__vlaPremium=true;renderAll=wrapped}
  }
  function boot(){if(document.documentElement.dataset.vlaPremiumBooted)return;document.documentElement.dataset.vlaPremiumBooted='1';addShell();wrapRender();setTimeout(()=>{premiumRender();premiumPorton()},80)}
