(function () {
  ...
  function openProfile(ownerId, requestedPlan, proposedEffectiveDate, sourceRequestId) {
    ...
    function updateProjectedCounts() {
      var total = Number(summary.totalOwners || (plantData.houses || []).length), host = modal.querySelector('#plant-projected-counts'), policy = planPolicies[form.elements.planId.value];
      ...
      host.innerHTML = '<span>Así quedará el conteo:</span>' + projections.map(function (entry) { var projected = Number(summary[entry[2]] || 0) - (profile[entry[1]] ? 1 : 0) + (policy[entry[0]] ? 1 : 0); return '<b>' + esc(entry[3]) + ' ' + projected + '/' + total + '</b>'; }).join('');
    }
