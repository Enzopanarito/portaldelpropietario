(function () {
  ...
  function ownerName(ownerId, house) {
    try { var item = owners.find(function (owner) { return owner.id === ownerId; }); return item ? item.Propietario : 'Casa ' + house; } catch (_) { return 'Casa ' + house; }
  }
  function tomorrow() { var date = new Date(); date.setUTCDate(date.getUTCDate() + 1); return date.toLocaleDateString('en-CA', { timeZone: 'America/Caracas' }); }
  function ensureUi() {
  ...
  function houseRows(houses) {
    return houses.map(function (item) {
      ...
      return '<tr><td><b>Casa ' + esc(item.house) + '</b><span>' + esc(ownerName(item.ownerId, item.house)) + '</span></td><td><b>' + esc(text(profile.state)) + '</b><span>Desde ' + esc(profile.effectiveFrom) + '</span></td><td><span>Rep. ' + (profile.participaReparaciones ? '✓' : '—') + ' · Mant. ' + (profile.participaMantenimiento ? '✓' : '—') + ' · Gasoil ' + (profile.participaGasoilResidencial ? '✓' : '—') + '</span><b>' + (profile.servicioResidencialActivo ? 'Servicio activo' : 'Servicio no activo') + '</b></td><td><b>' + money(reinstatement.total) + '</b><span>' + esc(text(reinstatement.mode)) + '</span></td><td><button class="plant-profile-simulate" data-owner="' + esc(item.ownerId) + '">Simular</button><button class="plant-profile-edit" data-owner="' + esc(item.ownerId) + '">Nueva versión</button></td></tr>';
    }).join('');
  }
  ...
  function render(data) {
    ...
    document.getElementById('plant-module-body').innerHTML =
      ...
      '<div id="plant-participation" class="plant-admin-panel"><div class="plant-panel-title"><div><h3>Participación por casa</h3><p>Perfiles vigentes y simulador read-only. Cada cambio crea una versión futura.</p></div></div><div class="plant-table-wrap"><table><thead><tr><th>Casa</th><th>Estado</th><th>Reglas</th><th>Reincorporación</th><th></th></tr></thead><tbody>' + houseRows(data.houses || []) + '</tbody></table></div></div>' +
  ...
  function openProfile(ownerId) {
    ...
    modal.querySelector('form').addEventListener('submit', async function (event) {
      event.preventDefault(); var form = event.currentTarget, button = form.querySelector('button'); button.disabled = true; button.textContent = 'Guardando…';
      try {
        var f = new FormData(form), result = await api(endpoint, { method: 'POST', body: JSON.stringify({ action: 'create-profile-version', ownerId: ownerId, effectiveFrom: f.get('effectiveFrom'), reason: f.get('reason'), reinstatementRequestId: f.get('reinstatementRequestId'), profile: { state: f.get('state'), reinstatementMode: f.get('reinstatementMode'), participaReparaciones: f.has('participaReparaciones'), participaMantenimiento: f.has('participaMantenimiento'), participaGasoilResidencial: f.has('participaGasoilResidencial'), participaBeneficioComun: f.has('participaBeneficioComun'), servicioResidencialActivo: f.has('servicioResidencialActivo'), specialAgreement: f.has('specialAgreement'), observations: f.get('observations') } }) });
        notice(result.message); modal.remove(); plantData = null; loadPlant(true);
      } catch (error) { notice(error.message, true); button.disabled = false; button.textContent = 'Programar versión'; }
    });
  }
