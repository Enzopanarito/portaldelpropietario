...
test('la prelectura detecta un SHA duplicado antes del envío final',async()=>{const {exactDuplicateStatus}=loadWithAnalysis(base()),request=async url=>({ok:true,json:async()=>({records:url.includes('Reportes%20de%20Pago')?[{id:'recDUPLICATE00001'}]:[]})}),result=await exactDuplicateStatus('a'.repeat(64),request,{baseId:'appTEST',apiToken:'patTEST'});assert.equal(result.checked,true);assert.equal(result.duplicate,true)});
test('consulta reportes y pagos en paralelo para no retrasar la previsualización',async()=>{const {exactDuplicateStatus}=loadWithAnalysis(base());let active=0,peak=0;const request=async()=>{active+=1;peak=Math.max(peak,active);await new Promise(resolve=>setTimeout(resolve,10));active-=1;return{ok:true,json:async()=>({records:[]})}};const result=await exactDuplicateStatus('b'.repeat(64),request,{baseId:'appTEST',apiToken:'patTEST'});assert.equal(result.duplicate,false);assert.equal(peak,2)});
