...
test('prioriza Flash-Lite y cambia a 3.6 cuando el lector rápido no está disponible',async()=>{const calls=[],runnerFactory=()=>async({model})=>{calls.push(model);if(model==='gemini-3.5-flash-lite')throw Object.assign(new Error('modelo rápido temporalmente no disponible'),{code:'PROVIDER_UNAVAILABLE',status:503});return JSON.stringify(base())};const {handler}=loadWithAnalysis(base(),{runnerFactory}),response=await handler(event());assert.equal(response.statusCode,200);assert.deepEqual(calls,['gemini-3.5-flash-lite','gemini-3.6-flash'])});
