...
test('usa automáticamente la fecha del archivo cuando la IA no encuentra una visible',async()=>{const modified=Date.now()-86400000,{handler}=loadWithAnalysis(base({transaction_date:null,critical_fields_visible:false})),response=await handler(event({base64:'x',lastModified:modified})),body=JSON.parse(response.body);assert.equal(response.statusCode,200);assert.equal(body.complete,true);assert.equal(body.analysis.transactionDateSource,'FILE_LAST_MODIFIED');assert.equal(body.analysis.transactionDateConfidence,'MEDIUM');assert.equal(body.analysis.transactionDateNeedsReview,true)});
test('Zelle sin fecha ignora lastModified y usa recepción Caracas',async()=>{const modified=Date.now()-86400000,{handler}=loadWithAnalysis(base({method:'ZELLE',bank_or_platform:'Zelle',transaction_date:null,critical_fields_visible:false})),response=await handler(event({base64:'x',lastModified:modified})),body=JSON.parse(response.body);assert.equal(response.statusCode,200);assert.equal(body.analysis.transactionDateSource,'REPORT_TIMESTAMP_FALLBACK');assert.equal(body.analysis.transactionDateNeedsReview,false);assert.equal(body.analysis.transactionDateAttestation,'')});
test('una fecha visible recibe atestación firmada del servidor',async()=>{const {handler}=loadWithAnalysis(base({method:'BINANCE_PAY',bank_or_platform:'Binance Pay',transaction_date:'2026-08-07'})),response=await handler(event()),body=JSON.parse(response.body);assert.equal(response.statusCode,200);assert.equal(body.analysis.transactionDateSource,'PROOF_EXTRACTED');assert.match(body.analysis.transactionDateAttestation,/^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/)});
