...
test('fuerza Zelle y Binance a USD aunque la IA devuelva otra moneda',async()=>{for(const method of ['ZELLE','TRANSFER_US','BINANCE_PAY','CRYPTO_TRANSFER']){const {handler}=loadWithAnalysis(base({method,bank_or_platform:method,currency:'VES'})),response=await handler(event()),body=JSON.parse(response.body);assert.equal(response.statusCode,200);assert.equal(body.analysis.currency,'USD');assert.equal(body.complete,true)}});
test('fuerza transferencias venezolanas y pago móvil a VES',async()=>{for(const method of ['TRANSFER_VE','MOBILE_PAYMENT_VE']){const {handler}=loadWithAnalysis(base({method,bank_or_platform:method,currency:'USD'})),response=await handler(event()),body=JSON.parse(response.body);assert.equal(response.statusCode,200);assert.equal(body.analysis.currency,'VES');assert.equal(body.complete,true)}});
