...
(()=>{
  ...
  const devCommand=(launcher.split('\n').find(line=>line.includes(' dev --context production '))||'').trim();
  assert.match(devCommand,/dev --context production --port 8888/,'Netlify Dev debe usar el contexto production para inyectar secretos protegidos.');
  assert.doesNotMatch(devCommand,/--host\b/,'Netlify CLI v27 no soporta --host en netlify dev.');
