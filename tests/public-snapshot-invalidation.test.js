...
for(const source of ['admin-manual-payment','process-payment-report','admin-expense','batch-delete-records','monthly-close-v2']){
 assert(PUBLIC_SNAPSHOT_MUTATION_SOURCES.has(source),`${source} debe invalidar la fotografía.`);
}
...
assert(publicV3.includes("if(!isEnabled())return previousHandler(event)"),'La bandera apagada debe conservar la ruta anterior.');
...
assert(ownerHtml.includes('/.netlify/functions/public-data?force=1'),'El archivo productivo conserva el comportamiento histórico hasta activar la bandera.');
assert(!ownerEdge.includes("split('/.netlify/functions/public-data?force=1')"),'El Edge no debe cambiar el comportamiento con la bandera apagada.');
