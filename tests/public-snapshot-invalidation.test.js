...
assert(publicV3.includes('if(!isEnabled(snapshotEnv,snapshotStore.runtimeConfig,host)){'),'La bandera y el host deben omitir Blobs antes de cualquier lectura.');
assert(publicV3.includes('const direct=await previousHandler(event),payload=parseBody(direct)'),'El rollback conserva la ruta financiera canónica.');
assert(publicV3.includes('return financialUnavailable'),'Un contrato incompleto debe fallar cerrado aun con el caché apagado.');
...
assert(ownerHtml.includes('/api/vla/public-data?force=1'),'El portal debe usar la ruta moderna con contexto fuerte de Blobs.');
