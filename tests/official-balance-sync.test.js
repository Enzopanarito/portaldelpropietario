...
for (let house=1; house<=15; house+=1) {
  ...
  assert.strictEqual(publicResult.currentTotalRef, total, `Casa ${house}: saldo corriente`);
...
assert.strictEqual(paid.currentTotalRef, 278.17);
...
assert.strictEqual(charged.expiredTotalRef,0);
assert.strictEqual(charged.currentTotalRef,316.40);
...
assert.strictEqual(release.release, '2026-08-15-v13');
