...
assert(background.includes('consumedCommits'));
assert(background.includes("errorCode: 'COMMIT_ALREADY_CONSUMED'"));
...
assert(commitBody.indexOf('prepared.delete(attemptId)')<commitBody.indexOf('button.click()'),'La autorización debe consumirse antes del clic.');
...
assert(install.includes('Identidad estable de extensión verificada'));
assert(install.indexOf('swift test')<install.indexOf('Creando respaldo transaccional'));
...
assert(rollback.includes('inventory.tsv'));
...
assert(runner.includes('heartbeat'));
