...
const{EXPECTED_HOUSES,validatePayload,buildSnapshot,createSnapshotStore,namespace,snapshotKey,snapshotExpectedEtag,enabled,dataEnvironment,normalizeHost,requestHost,isProductionHost,environmentForEvent}=require('../netlify/functions/_shared/_public_snapshot_store');
...
function owner(index){const usd=index===0?85:0,bs=index===2?157.07:0,net=usd+bs;return{id:`recOwner${String(index+1).padStart(9,'0')}`,Casa:index+1,Propietario:`Casa ${index+1}`,'Saldo USD Actual':usd,'Saldo Bs Ref Actual':bs,'Saldo Total Actual':net,'Deuda Restante':net,saldoUsd:usd,saldoBsRef:bs,totalPagadero:Math.max(0,usd)+Math.max(0,bs),saldoNetoReferencial:net,balanceEngineVersion:'vla-balance-contract-v7'}}
function payload(){return{generatedAt:'2026-07-13T06:00:00.000Z',generatedAtCaracas:'13/07/2026, 02:00:00',balanceEngineVersion:5,officialBalanceSource:'ControlVersiones',propietarios:Array.from({length:EXPECTED_HOUSES},(_,index)=>owner(index)),gastos:[],pagos:[]}}
...
(async()=>{
 ...
 const legacyContract=payload();delete legacyContract.propietarios[0].balanceEngineVersion;assert.strictEqual(validatePayload(legacyContract).ok,false,'Una casa sin contrato v7 debe rechazarse, aunque los números parezcan consistentes.');
