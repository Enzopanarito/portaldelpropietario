...
const{EXPECTED_HOUSES,validatePayload,buildSnapshot,createSnapshotStore,namespace,snapshotKey,snapshotExpectedEtag}=require('../netlify/functions/_public_snapshot_store');
...
(async()=>{
 const env={CONTEXT:'production',VLA_DATA_ENVIRONMENT:'production',AIRTABLE_BASE_ID:'appPRODUCTION0001',PUBLIC_BLOB_CACHE_ENABLED:'true',PUBLIC_BLOB_CACHE_MAX_AGE_MS:'120000'},valid=payload();
 ...
 let clock=1_000_000;const fake=new FakeStore(),snapshots=createSnapshotStore({storeFactory:async()=>fake,now:()=>clock});
