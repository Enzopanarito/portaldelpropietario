...
test('runtime local, build Netlify y Actions usan Node 24',()=>{assert.equal(packageJson.engines.node,'>=22 <25');assert.equal(fs.readFileSync('.nvmrc','utf8').trim(),'24');assert.match(fs.readFileSync('netlify.toml','utf8'),/NODE_VERSION\s*=\s*"24"/);const offenders=workflowSources.filter(item=>/node-version:\s*['"]?(?:20|22)['"]?/.test(item.source)).map(item=>item.name);assert.deepEqual(offenders,[],`Workflows fuera de Node 24: ${offenders.join(', ')}`);assert.match(fs.readFileSync('.github/workflows/netlify-production.yml','utf8'),/AWS_LAMBDA_JS_RUNTIME:\s*nodejs24\.x/)});
