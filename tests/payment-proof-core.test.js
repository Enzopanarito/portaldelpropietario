...
function jpeg(width=1200,height=900,{complete=true}={}){
 const soi=Buffer.from([0xff,0xd8,0xff,0xe0,0x00,0x10]),app=Buffer.alloc(14),sof=Buffer.alloc(19);sof[0]=0xff;sof[1]=0xc0;sof.writeUInt16BE(17,2);sof[4]=8;sof.writeUInt16BE(height,5);sof.writeUInt16BE(width,7);sof[9]=3;
 return Buffer.concat([soi,app,sof,Buffer.alloc(256,0xaa),complete?Buffer.from([0xff,0xd9]):Buffer.alloc(0)]);
}
...
function isoImage(brand='heic',{declared=32}={}){const buffer=Buffer.alloc(32);buffer.writeUInt32BE(declared,0);buffer.write('ftyp',4,'ascii');buffer.write(brand,8,'ascii');buffer.write(brand,16,'ascii');return buffer}
...
(()=>{
 ...
 const iphone=core.decodeProofInput(input('IMG_0001.HEIC','image/heic',isoImage('heic')));assert.strictEqual(iphone.requiresConversion,true);assert.strictEqual(iphone.format,'heic');
 const genericHeif=core.decodeProofInput(input('IMG_0002.HEIF','image/heif',isoImage('mif1')));assert.strictEqual(genericHeif.format,'heif');
 ...
 for(const [name,type,content] of [['cut.png','image/png',png(900,900,{complete:false})],['cut.jpg','image/jpeg',jpeg(900,900,{complete:false})],['cut.webp','image/webp',webp(900,900,{complete:false})],['cut.pdf','application/pdf',pdf(false)],['cut.heic','image/heic',isoImage('heic',{declared:64})]])assert.strictEqual(core.decodeProofInput(input(name,type,content)).quality.reason,'TRUNCATED_FILE');
 ...
 assert.throws(()=>core.decodeBase64Strict('Zh=='),error=>error.code==='INVALID_BASE64','Base64 con bits de relleno no canónicos debe rechazarse.');
 ...
 assert.throws(()=>core.buildIdempotencyKey('rec123','not-a-sha','PROMPT_V2'),/attachmentSha/);
