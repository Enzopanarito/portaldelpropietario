...
test('preview devuelve fixture y nunca consulta Airtable',async()=>{
  let reads=0;
  const handler=createHandler({env:{CONTEXT:'deploy-preview'},getAll:async()=>{reads++;throw new Error('no debe leer Airtable')},now:()=>new Date('2026-08-25T12:00:00-04:00')});
...
test('producción solo ejecuta lecturas y entrega contrato read-only',async()=>{
  ...
  const handler=createHandler({env:{CONTEXT:'production',AIRTABLE_API_TOKEN:'test-token',AIRTABLE_BASE_ID:'app4nE4ReGRi2SuP2'},publicHandler,getAll,buildPunctualityScore,now:()=>new Date('2026-08-25T12:00:00-04:00')});
  const result=await handler({httpMethod:'GET',queryStringParameters:{ownerId:OWNER_ID}}),body=JSON.parse(result.body);
...
test('un fallo del índice falla suave y no contamina el estado de cuenta',async()=>{
  const handler=createHandler({env:{CONTEXT:'production',AIRTABLE_API_TOKEN:'test-token',AIRTABLE_BASE_ID:'app4nE4ReGRi2SuP2'},publicHandler:async()=>response(500,{message:'fallo'}),getAll:async()=>[],previewMode:()=>false});
  const result=await handler({httpMethod:'GET',queryStringParameters:{ownerId:OWNER_ID}}),body=JSON.parse(result.body);
...
test('rechaza cualquier método de escritura',async()=>{
  const handler=createHandler({env:{CONTEXT:'production'},previewMode:()=>false});
