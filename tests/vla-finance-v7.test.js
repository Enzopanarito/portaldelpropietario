...
function canonical(usd,bs,overrides={}){return{saldoUsd:usd,saldoBsRef:bs,totalPagadero:finance.money(Math.max(0,usd)+Math.max(0,bs)),saldoNetoReferencial:finance.money(usd+bs),saldoFavorUsd:finance.money(Math.max(0,-usd)),saldoFavorBs:finance.money(Math.max(0,-bs)),deudaVencidaUsd:0,deudaVencidaBs:0,mesCorrienteUsd:usd,mesCorrienteBs:bs,balanceEngineVersion:finance.VERSION,...overrides}}
