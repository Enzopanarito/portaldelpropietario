...
function payment({id='recPay',ownerId='recOwner4',date='2026-07-12',createdTime='2026-07-12T15:00:00.000Z',mode='USD',equivalent=10}={}){return{id,createdTime,fields:{'Propietario que Paga':[ownerId],'Fecha de Pago':date,'Forma de Pago':mode,'Monto Pagado':mode==='USD'?equivalent:0,'Monto Pagado Bs':mode==='Bs BCV'?equivalent*50:0,'Tasa BCV Aplicada':50,'Equivalente USD Aplicado':equivalent,'[x] Aplicado al Cierre':false}}}
...
(()=>{
 ...
 const laterCapture=snapshot.buildAccessSnapshot({owner:owner(),payments:[],officialRecords:[record],bcvRate:50,bcvSource:'BCV 2026-07-13',now:new Date('2026-07-13T13:00:00.000Z')});assert.strictEqual(laterCapture.snapshotId,built.snapshotId,'La hora de captura no debe cambiar la identidad del mismo estado financiero.');assert.notStrictEqual(laterCapture.capturedAt,built.capturedAt);
 ...
 const beforeCustomDue=snapshot.buildAccessSnapshot({owner:owner(),payments:[],officialRecords:[record],bcvRate:50,now,dueDay:15,surchargeRate:.05});
 assert.strictEqual(beforeCustomDue.dueDay,15);assert.strictEqual(beforeCustomDue.surchargeRate,.05);assert.strictEqual(beforeCustomDue.expiredTotalUsd,185,'La deuda anterior permanece vencida aunque cambie el vencimiento del mes corriente.');assert.notStrictEqual(beforeCustomDue.snapshotId,built.snapshotId);

 const after=payment(),changed=snapshot.buildAccessSnapshot({owner:owner(),payments:[after],officialRecords:[record],bcvRate:50,now});
 ...
 const bsPayment=payment({id:'recBs',date:'2026-07-10',createdTime:'2026-07-10T12:00:00.000Z',mode:'Bs BCV',equivalent:20}),matchingAfterBs=current({usd:85,bs:80});
 ...
 const expenseOwner=owner({id:'recOwner6',house:6,usd:0,bs:0,aliquot:1}),expense={id:'recExpense',fields:{Concepto:'Mantenimiento','Monto':50,'Tipo de Gasto':'Gasto Común','Forma de Pago':'USD',Propietarios:['recOwner6']}},expenseOfficial=current({house:6,usd:50,bs:0,id:'recExpenseOfficial'});
 const withExpense=snapshot.buildAccessSnapshot({owner:expenseOwner,expenses:[expense],payments:[],officialRecords:[expenseOfficial],bcvRate:50,now});assert.strictEqual(withExpense.cacheValid,true,'El saldo vivo debe incluir los gastos del mes.');assert.strictEqual(withExpense.source,'ControlVersiones');assert.strictEqual(withExpense.expiredTotalUsd,0,'Terminar el pronto pago no convierte el gasto corriente en deuda vencida.');assert.strictEqual(withExpense.automaticEligibility,false,'La cuota corriente no puede habilitar ni limitar el portón.');
 const currentBeforeDue=snapshot.buildAccessSnapshot({owner:expenseOwner,expenses:[expense],payments:[],officialRecords:[expenseOfficial],bcvRate:50,now,dueDay:15,surchargeRate:.05});assert.strictEqual(currentBeforeDue.expiredTotalUsd,0,'El cargo del mes todavía no está vencido.');assert.strictEqual(currentBeforeDue.automaticEligibility,false);
 const withoutExpense=snapshot.buildAccessSnapshot({owner:expenseOwner,expenses:[],payments:[],officialRecords:[expenseOfficial],bcvRate:50,now});assert.strictEqual(withoutExpense.cacheValid,false);assert(withoutExpense.invalidReasons.includes('OFFICIAL_BALANCE_MISMATCH'));
 const currentExpenseValidation=snapshot.validateSnapshotStillCurrent(withExpense,{owner:expenseOwner,expenses:[expense],payments:[],officialRecords:[expenseOfficial],bcvRate:50,now});assert.strictEqual(currentExpenseValidation.ok,false);assert.strictEqual(currentExpenseValidation.reason,'SNAPSHOT_NOT_AUTOMATICALLY_ELIGIBLE');
