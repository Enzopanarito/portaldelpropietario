...
(async()=>{
 const rejected=await processReport.handler(event({reportId:rejectId,decision:'reject',reason:''}));assert.strictEqual(rejected.statusCode,200);assert.strictEqual(parse(rejected).access.unchanged,true);assert.strictEqual(syncCount,0,'Rechazar nunca puede sincronizar ni cambiar acceso.');assert.strictEqual(createCount,0);assert.equal(Object.hasOwn(reports.get(rejectId).fields,'Motivo del Rechazo'),false);
 ...
 const corrected=await processReport.handler(event({reportId:correctionId,decision:'correct_and_approve',reason:'',corrections:{reference:'REF-CORREGIDA'}}));assert.strictEqual(corrected.statusCode,200,JSON.stringify(parse(corrected)));assert.strictEqual(reports.get(correctionId).fields.Estado,'Confirmado');assert.strictEqual(reports.get(correctionId).fields.Referencia,'REF-CORREGIDA');assert.strictEqual(createCount,2);assert.strictEqual(syncCount,2);
 const exception=await processReport.handler(event({reportId:exceptionId,decision:'approve_exception',reason:''}));assert.strictEqual(exception.statusCode,200,JSON.stringify(parse(exception)));assert.strictEqual(reports.get(exceptionId).fields['Decisión Administrativa'],'Aprobado por excepción');assert.equal(Object.hasOwn(reports.get(exceptionId).fields,'Motivo de Excepción'),false);assert.strictEqual(createCount,3);assert.strictEqual(syncCount,3);
