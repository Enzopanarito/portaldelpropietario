...
const expense = { id:'recEXPENSE000001', fields:{Concepto:'Condominio',Monto:100,'Tipo de Gasto':'Gasto Común','Forma de Pago':'Bs BCV',Propietarios:[ownerA.id,ownerB.id]} };
const payment = { id:'recPAYMENT000001', fields:{'Propietario que Paga':[ownerA.id],'Monto Pagado':15,'Equivalente USD Aplicado':15,'Forma de Pago':'Bs BCV','Fecha de Pago':'2026-07-05','[x] Aplicado al Cierre':false} };
...
assert.strictEqual(plan.planHash,reversed.planHash,'La huella debe ser determinista.');
assert.strictEqual(plan.paymentIds.length,1);
assert.strictEqual(plan.validation.pendingPaymentsCount,1);
assert.strictEqual(plan.ownerUpdates.find(x=>x.id===ownerA.id).target.deudaAnterior,45);
assert.strictEqual(plan.ownerUpdates.find(x=>x.id===ownerB.id).target.deudaAnterior,50);
const changed=JSON.parse(JSON.stringify(payment));changed.fields['Monto Pagado']=16;changed.fields['Equivalente USD Aplicado']=16;
assert.notStrictEqual(plan.planHash,core.buildPlan({owners:[ownerA,ownerB],expenses:[expense],payments:[changed],month:'2026-07'}).planHash,'Un cambio financiero debe invalidar la simulación.');
const fields=core.debtFields(plan.ownerUpdates[0].target);
assert(Object.prototype.hasOwnProperty.call(fields,'Deuda Anterior USD'));
assert(Object.prototype.hasOwnProperty.call(fields,'Deuda Anterior Bs Ref'));
assert(Object.prototype.hasOwnProperty.call(fields,'Deuda Anterior'));
...
assert(adminHtml.includes('async function adminFetch('));
assert(adminHtml.includes('async function runClose('));
assert(adminHtml.includes('/.netlify/functions/monthly-close'));
assert(adminHtml.includes('/.netlify/functions/audit-snapshot'));
assert(adminHtml.includes('planHash:finalCheck.planHash'));
assert(adminHtml.includes("action:'repair'"));
assert(!adminHtml.includes('/api/vla/monthly-close'));
assert(adminEdge.includes('planHash:finalCheck.planHash'));
assert(adminEdge.includes("action:'repair'"));
assert(adminEdge.includes('finalCheck.planHash!==dry.planHash'));
assert(adminEdge.includes('audit-snapshot'));
