...
async function live(browser,target){
  ...
  assert(response.headers()['x-vla-owner-payment-report']==='smart-v6','Falta marcador smart-v6.');
  ...
  assert(!errors.length,`Errores live: ${errors.join(' | ')}`);await page.close();return{metrics,detection,casa2AccountsVerified:Boolean(casa2),errors}
}
...
async function fixture(browser){
  ...
  await page.route('**/api/vla/payment-proof-prefill',async route=>route.fulfill({status:200,contentType:'application/json',body:'{"analysis":{"currency":"UNKNOWN","confidence":0},"complete":false,"missing":[{"label":"moneda"},{"label":"monto"},{"label":"referencia"}]}' }));
  ...
  await page.locator('html[data-vla-owner-payment-report="smart-v6"]').waitFor({state:'attached',timeout:10000});await page.click('#reportBtn');await chooseChannel(page,'Efectivo','#payChannelCash');await page.locator('#vla-pay-details').waitFor({state:'visible',timeout:10000});
  const initial=await page.evaluate(()=>{const a=submitReport.getBoundingClientRect(),b=cancelModal.getBoundingClientRect();return{text:modal.innerText,validation:document.getElementById('vla-pay-validation').innerText,gap:b.top-a.bottom,width:document.documentElement.scrollWidth,viewport:innerWidth,proofDisabled:payProof.disabled,submitDisabled:submitReport.disabled,submitAriaDisabled:submitReport.getAttribute('aria-disabled')}});
  assert(initial.proofDisabled&&!initial.submitDisabled&&initial.submitAriaDisabled==='true'&&/moneda/i.test(initial.validation)&&/monto/i.test(initial.validation)&&/a quién o dónde/i.test(initial.validation),'El efectivo no anuncia correctamente sus datos obligatorios.');assert(initial.gap>=12,'Botones juntos.');assert(initial.width<=initial.viewport+2,'Desbordamiento.');assert(!/recargo/i.test(initial.text),'Muestra recargo.');
  ...
  await page.fill('#payRef','ABC-12345');await page.fill('#payBank','Pago móvil');await page.fill('#payTransactionDate','2026-07-12');await page.selectOption('#payTransactionStatus','COMPLETED');await page.locator('#submitReport[aria-disabled="false"]').waitFor({state:'visible',timeout:10000});await page.screenshot({path:'owner-payment-report-mobile.png'});await page.click('#submitReport');await page.locator('#modal.hidden').waitFor({state:'attached',timeout:10000});
