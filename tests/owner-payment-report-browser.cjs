...
async function waitForText(page,locator,pattern,timeout=10000){const deadline=Date.now()+timeout;while(Date.now()<deadline){const value=await locator.innerText().catch(()=>'');if(pattern.test(value))return value;await page.waitForTimeout(200)}throw new Error(`No apareció el texto esperado: ${pattern}.`)}
...
async function live(browser,target){
  ...
  await page.locator('#payChannelCash').check({force:true});await page.locator('#vla-pay-details').waitFor({state:'visible',timeout:10000});
  ...
  await waitForText(page,page.locator('#vla-pay-detection'),/Monto identificado: Bs/);
  ...
  if(casa2){await page.selectOption('#userSelector',casa2);await page.dispatchEvent('#userSelector','change');await page.click('#reportBtn');await page.locator('#payChannelCash').check({force:true});await page.locator('#vla-pay-details').waitFor({state:'visible',timeout:10000});const options=await page.locator('#payMode option').allTextContents();assert(options.some(x=>x.includes('Adelanto para la cuenta USD'))&&options.some(x=>x.includes('Adelanto para la cuenta Bs')),'Casa solvente no permite adelantos.');await page.click('#cancelModal')}
...
async function fixture(browser){
  ...
  await page.locator('html[data-vla-owner-payment-report="smart-v5"]').waitFor({state:'attached',timeout:10000});await page.click('#reportBtn');await page.locator('#payChannelCash').check({force:true});await page.locator('#vla-pay-details').waitFor({state:'visible',timeout:10000});
  ...
  await page.locator('#payChannelDigital').check({force:true});const png=Buffer.concat([Buffer.from([137,80,78,71,13,10,26,10]),Buffer.from('fixture')]);await page.setInputFiles('#payProof',{name:'comprobante.png',mimeType:'image/png',buffer:png});await page.locator('#vla-pay-details').waitFor({state:'visible',timeout:10000});
  await page.selectOption('#payCurrency','BS');await page.selectOption('#payMode','USD');await page.fill('#payAmount','15.300,00');await page.locator('#payAmount').blur();await waitForText(page,page.locator('#vla-pay-detection'),/Monto identificado: Bs/);
  await page.fill('#payRef','ABC-12345');await page.fill('#payBank','Pago móvil');await page.fill('#payTransactionDate','2026-07-12');await page.selectOption('#payTransactionStatus','COMPLETED');await page.screenshot({path:'owner-payment-report-mobile.png'});await page.click('#submitReport');await page.locator('#modal').waitFor({state:'hidden',timeout:10000});
  ...
  await page.evaluate(()=>{current={debtUsd:0,debtBs:0,total:0,bsDue:0}});await page.click('#reportBtn');await page.locator('#payChannelCash').check({force:true});await page.locator('#vla-pay-details').waitFor({state:'visible',timeout:10000});const options=await page.locator('#payMode option').allTextContents();assert(options.some(x=>x.includes('Adelanto para la cuenta USD'))&&options.some(x=>x.includes('Adelanto para la cuenta Bs')),'No permite adelantos.');
