...
async function live(browser,target){
  ...
  assert(response.headers()['x-vla-owner-payment-report']==='progressive-v7','Falta marcador progressive-v7.');
...
async function fixture(browser){
  ...
  await page.locator('html[data-vla-owner-payment-report="progressive-v7"]').waitFor({state:'attached',timeout:10000});await page.click('#reportBtn');await chooseChannel(page,'Efectivo','#payChannelCash');await page.locator('#vla-pay-details').waitFor({state:'visible',timeout:10000});
