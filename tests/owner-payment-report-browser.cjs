...
async function fixture(browser){
  ...
  await page.fill('#payRef','ABC-12345');await page.fill('#payBank','Pago móvil');await page.fill('#payTransactionDate','2026-07-12');await page.selectOption('#payTransactionStatus','COMPLETED');await page.locator('#submitReport:not([disabled])').waitFor({state:'visible',timeout:10000});await page.screenshot({path:'owner-payment-report-mobile.png'});await page.click('#submitReport');await page.locator('#modal.hidden').waitFor({state:'attached',timeout:10000});
