...
async function fixture(browser){
  ...
  await page.locator('html[data-vla-owner-payment-report="smart-v5"]').waitFor({state:'attached',timeout:10000});await page.click('#reportBtn');
  ...
  await page.selectOption('#payMode','USD');await page.fill('#payAmount','15.300,00');await page.locator('#payAmount').blur();await waitForText(page,page.locator('#vla-pay-detection'),/Monto identificado: Bs/);
  await page.fill('#payRef','ABC-12345');await page.fill('#payBank','Pago móvil');const png=Buffer.concat([Buffer.from([137,80,78,71,13,10,26,10]),Buffer.from('fixture')]);await page.setInputFiles('#payProof',{name:'comprobante.png',mimeType:'image/png',buffer:png});await page.screenshot({path:'owner-payment-report-mobile.png'});await page.click('#submitReport');await page.locator('#modal').waitFor({state:'hidden',timeout:10000});
