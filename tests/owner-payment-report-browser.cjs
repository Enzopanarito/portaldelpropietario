...
async function live(browser,target){
  ...
  await page.locator('#payChannelCash').check();await page.locator('#vla-pay-details').waitFor({state:'visible',timeout:10000});
  ...
  if(casa2){await page.selectOption('#userSelector',casa2);await page.dispatchEvent('#userSelector','change');await page.click('#reportBtn');await page.locator('#payChannelCash').check();await page.locator('#vla-pay-details').waitFor({state:'visible',timeout:10000});const options=await page.locator('#payMode option').allTextContents();assert(options.some(x=>x.includes('Adelanto para la cuenta USD'))&&options.some(x=>x.includes('Adelanto para la cuenta Bs')),'Casa solvente no permite adelantos.');await page.click('#cancelModal')}
...
async function fixture(browser){
  ...
  await page.locator('html[data-vla-owner-payment-report="smart-v5"]').waitFor({state:'attached',timeout:10000});await page.click('#reportBtn');await page.locator('#payChannelCash').check();await page.locator('#vla-pay-details').waitFor({state:'visible',timeout:10000});
  ...
  await page.locator('#payChannelDigital').check();const png=Buffer.concat([Buffer.from([137,80,78,71,13,10,26,10]),Buffer.from('fixture')]);await page.setInputFiles('#payProof',{name:'comprobante.png',mimeType:'image/png',buffer:png});await page.locator('#vla-pay-details').waitFor({state:'visible',timeout:10000});
  ...
  await page.evaluate(()=>{current={debtUsd:0,debtBs:0,total:0,bsDue:0}});await page.click('#reportBtn');await page.locator('#payChannelCash').check();await page.locator('#vla-pay-details').waitFor({state:'visible',timeout:10000});const options=await page.locator('#payMode option').allTextContents();assert(options.some(x=>x.includes('Adelanto para la cuenta USD'))&&options.some(x=>x.includes('Adelanto para la cuenta Bs')),'No permite adelantos.');
