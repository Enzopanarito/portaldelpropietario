...
async function live(browser,target){
  ...
  assert(response.headers()['x-vla-owner-payment-report']==='progressive-v11','Falta marcador progressive-v11.');
...
async function fixture(browser){
  ...
  await page.locator('html[data-vla-owner-payment-report="progressive-v11"]').waitFor({state:'attached',timeout:10000});await page.click('#reportBtn');assert(await page.locator('#payChannelDigital').isChecked(),'Digital debe abrir preseleccionado.');await chooseChannel(page,'Efectivo','#payChannelCash');await page.locator('#vla-pay-details').waitFor({state:'visible',timeout:10000});
  ...
  await page.fill('#payRef','ABC-12345');await page.fill('#payBank','Pago móvil');assert(await page.locator('#vla-field-date').isHidden(),'La fecha automática no debe convertirse en una tarea manual.');await page.locator('#submitReport:not([disabled])').waitFor({state:'visible',timeout:10000});await page.screenshot({path:'owner-payment-report-mobile.png'});await page.click('#submitReport');await page.locator('#vla-pay-uncertainty-choice').waitFor({state:'visible',timeout:10000});assert(submitAttempts===0,'La duda no debe crear ni enviar el reporte antes de elegir.');await page.click('#vla-pay-uncertainty-review');assert(submitAttempts===0,'Elegir No, revisar no puede enviar el reporte.');await page.click('#submitReport');await page.locator('#vla-pay-uncertainty-choice').waitFor({state:'visible',timeout:10000});await page.click('#vla-pay-uncertainty-submit');await page.locator('#modal.hidden').waitFor({state:'attached',timeout:10000});
  assert(submitAttempts===2,'El envío no se recuperó de un 503 transitorio.');
  assert(payload?.mode==='USD'&&payload?.enteredCurrency==='BS'&&payload?.amount===15300,'Payload de moneda incorrecto.');assert(payload?.attachment?.type==='image/png'&&payload.attachment.base64&&payload.attachment.lastModified,'No envió comprobante y metadatos.');assert(payload?.transactionDate===''&&payload?.transactionDateSource==='UNDETERMINED','La carga no debe convertirse en fecha de pago.');assert(payload?.analysisSummary?.provider==='fixture-model'&&payload.analysisSummary.dateNeedsReview===true,'No transmitió al administrador la información de prelectura.');assert(payload?.uncertaintyAcknowledged===true,'No conservó la decisión explícita del propietario.');assert((await page.evaluate(()=>JSON.parse(localStorage.getItem('vla-payment-reports-v1:recABCDEFGHIJKLMN')||'[]').length))===1,'No guardó el seguimiento privado en el dispositivo.');
