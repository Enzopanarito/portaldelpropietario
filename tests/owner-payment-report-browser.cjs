...
async function live(browser,target){
  ...
  assert(response.headers()['x-vla-owner-payment-report']==='progressive-v6','Falta marcador progressive-v6.');
...
async function fixture(browser){
  ...
  await page.locator('html[data-vla-owner-payment-report="progressive-v6"]').waitFor({state:'attached',timeout:10000});await page.click('#reportBtn');await chooseChannel(page,'Efectivo','#payChannelCash');await page.locator('#vla-pay-details').waitFor({state:'visible',timeout:10000});
  ...
  await page.fill('#payRef','ABC-12345');await page.fill('#payBank','Pago móvil');await page.fill('#payTransactionDate','2026-07-12');await page.locator('#submitReport:not([disabled])').waitFor({state:'visible',timeout:10000});await page.screenshot({path:'owner-payment-report-mobile.png'});await page.click('#submitReport');await page.locator('#modal.hidden').waitFor({state:'attached',timeout:10000});
