...
async function live(browser,target){
  ...
  assert(response.headers()['x-vla-owner-payment-report']==='progressive-v12','Falta marcador progressive-v12.');
...
async function fixture(browser){
  ...
  await page.locator('html[data-vla-owner-payment-report="progressive-v12"]').waitFor({state:'attached',timeout:10000});await page.click('#reportBtn');assert(await page.locator('#payChannelDigital').isChecked(),'Digital debe abrir preseleccionado.');await chooseChannel(page,'Efectivo','#payChannelCash');await page.locator('#vla-pay-details').waitFor({state:'visible',timeout:10000});
