...
async function fixture(browser){
  ...
  await page.route('**/api/vla/payment-proof-prefill',async route=>route.fulfill({status:200,contentType:'application/json',body:'{"analysis":{"currency":"UNKNOWN","confidence":0,"transactionDate":"2026-07-12","transactionDateSource":"FILE_LAST_MODIFIED","transactionDateConfidence":"MEDIUM","transactionDateNeedsReview":true,"transactionDateEvidence":"Fecha estimada desde el archivo."},"analysisProvider":"fixture-model","analysisRoute":"fixture","complete":false,"missing":[{"label":"moneda"},{"label":"monto"},{"label":"referencia"}]}' }));
  ...
  await page.locator('html[data-vla-owner-payment-report="progressive-v8"]').waitFor({state:'attached',timeout:10000});await page.click('#reportBtn');await chooseChannel(page,'Efectivo','#payChannelCash');await page.locator('#vla-pay-details').waitFor({state:'visible',timeout:10000});
  ...
  await page.fill('#payRef','ABC-12345');await page.fill('#payBank','Pago móvil');assert(await page.locator('#vla-field-date').isHidden(),'La fecha automática no debe convertirse en una tarea manual.');await page.locator('#submitReport:not([disabled])').waitFor({state:'visible',timeout:10000});await page.screenshot({path:'owner-payment-report-mobile.png'});await page.click('#submitReport');await page.locator('#modal.hidden').waitFor({state:'attached',timeout:10000});
  assert(submitAttempts===2,'El envío no se recuperó de un 503 transitorio.');
  assert(payload?.mode==='USD'&&payload?.enteredCurrency==='BS'&&payload?.amount===15300,'Payload de moneda incorrecto.');assert(payload?.attachment?.type==='image/png'&&payload.attachment.base64&&payload.attachment.lastModified,'No envió comprobante y metadatos.');assert(payload?.transactionDate==='2026-07-12'&&payload?.transactionDateSource==='FILE_LAST_MODIFIED','No conservó la fecha automática con su trazabilidad.');assert(payload?.analysisSummary?.provider==='fixture-model'&&payload.analysisSummary.dateNeedsReview===true,'No transmitió al administrador la información de prelectura.');
