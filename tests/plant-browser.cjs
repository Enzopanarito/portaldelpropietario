...
function adminHtml() {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><style>body{margin:0;background:#f4f7f5;font-family:Arial}.section{display:none}.section.active{display:block}.hidden{display:none}#vla-premium-shell{max-width:1180px;margin:auto;padding:20px}#vla-premium-sidebar .vla-nav{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:18px}#vla-premium-sidebar button{padding:10px;border:0;border-radius:10px}#vla-premium-sidebar button.active{background:#12382c;color:white}</style><link rel="stylesheet" href="/admin-plant-v1.css"></head><body><div id="app"><div id="vla-premium-shell"><aside id="vla-premium-sidebar"><nav class="vla-nav"><button class="active" data-vla-target="dashboard">Dashboard</button><button data-vla-target="owners">Propietarios</button><button data-vla-target="expenses">Gastos</button><button data-vla-target="health">Salud</button></nav></aside><main id="vla-premium-main"><header><h1 id="vla-current-title">Portal Administrativo</h1></header><main id="vla-premium-content"><section id="dashboard" class="section active"></section><section id="expenses" class="section"><form id="expense-form"><input id="expense-concept"><input id="expense-amount"><div id="owners-checks"><div id="owners-list"></div></div><select id="expense-type"><option>Gasto Común</option><option>Gasto Especial</option></select><select id="expense-mode"><option>Bs BCV</option><option>USD</option></select><select id="expense-frequency"><option>Eventual</option><option>Fijo</option></select><button>Crear gasto</button></form></section><footer>VLA</footer></main></main></div></div><script>let owners=${JSON.stringify(publicOwners)};window.ready=true;async function adminFetch(url,opt){const r=await fetch(url,opt);const d=await r.json();if(!r.ok)throw new Error(d.message||'Error');return d}function toast(){}function caracasDate(){return '2026-08-21'}async function loadAll(){return true}</script><script src="/admin-plant-v1.js"></script></body></html>`;
}
...
for (const viewport of [{ name: 'desktop', width: 1365, height: 900 }, { name: 'mobile', width: 390, height: 844 }]) {
  test(`panel Admin planta funciona en ${viewport.name}`, async () => {
    ...
    assert.equal(result.menu, 'premium-sidebar'); assert.equal(result.title, 'Planta eléctrica'); assert.equal(result.active, true);
