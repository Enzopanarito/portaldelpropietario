...
(async()=>{
 ...
 const claimedLease={ok:true,key:'lease',lease:{operationId:'op'}};
 const refresh=createHandler({...baseDeps,enabled:()=>true,readPublicSnapshot:async env=>{assert.strictEqual(env,testEnv);return{ok:false,reason:'missing'}},claimPublicRefresh:async env=>{claimEnv=env;return claimedLease},releasePublicRefresh:async(_lease,env)=>{releaseEnv=env;releases+=1},writePublicSnapshot:async(payload,env,expectedVersion,leaseMarker)=>{writes+=1;assert.deepStrictEqual(payload,freshPayload);assert.strictEqual(env,testEnv);assert.deepStrictEqual(expectedVersion,{snapshotEtag:'',invalidationVersion:''});assert.strictEqual(leaseMarker,claimedLease)},previousHandler:async received=>{previousCalls+=1;assert.strictEqual(received.queryStringParameters.force,'1');return apiResponse(200,freshPayload,{'X-Airtable-Calls':'4'})}}),refreshResult=await refresh(event);assert.strictEqual(previousCalls,1);assert.strictEqual(writes,1);assert.strictEqual(releases,1);assert.strictEqual(claimEnv,testEnv);assert.strictEqual(releaseEnv,testEnv);assert.strictEqual(refreshResult.headers['X-Public-Snapshot'],'REFRESH');assert.strictEqual(refreshResult.headers['X-Airtable-Calls'],'4');
 ...
 let receivedVersion=null,receivedLease=null;
 const warningLease={ok:true,key:'lease',lease:{operationId:'op'}};
 const writeWarning=createHandler({...baseDeps,enabled:()=>true,readPublicSnapshot:async()=>snapshot(stalePayload,Date.now()-1,'etag-before-payment'),claimPublicRefresh:async()=>warningLease,releasePublicRefresh:async()=>{},writePublicSnapshot:async(_payload,env,expectedVersion,leaseMarker)=>{assert.strictEqual(env,testEnv);receivedVersion=expectedVersion;receivedLease=leaseMarker;const error=new Error('La fotografía cambió durante la reconstrucción.');error.code='STALE_PUBLIC_SNAPSHOT_WRITE';throw error},previousHandler:async()=>apiResponse(200,freshPayload)}),warningResult=await writeWarning(event);assert.deepStrictEqual(receivedVersion,{snapshotEtag:'etag-before-payment',invalidationVersion:''});assert.strictEqual(receivedLease,warningLease);assert.strictEqual(warningResult.statusCode,200);assert.strictEqual(warningResult.headers['X-Public-Snapshot'],'WRITE_WARNING');assert(warningResult.headers['X-Public-Snapshot-Warning'].includes('STALE_PUBLIC_SNAPSHOT_WRITE'));
