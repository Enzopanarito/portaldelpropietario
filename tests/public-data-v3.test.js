...
function canonicalOwner(index){return{Casa:index+1,'Saldo USD Actual':0,'Saldo Bs Ref Actual':0,'Saldo Total Actual':0,'Deuda Restante':0,saldoUsd:0,saldoBsRef:0,totalPagadero:0,saldoNetoReferencial:0,balanceEngineVersion:'vla-balance-contract-v7'}}
function canonicalPayload(){return{generatedAt:'2026-07-13T06:00:00.000Z',balanceEngineVersion:5,officialBalanceSource:'ControlVersiones',propietarios:Array.from({length:15},(_,index)=>canonicalOwner(index))}}
...
(async()=>{
 ...
 let previousCalls=0;
 const disabledPayload={...canonicalPayload(),legacy:true};
 const disabled=createHandler({...baseDeps,enabled:()=>false,previousHandler:async received=>{previousCalls+=1;assert.strictEqual(received,event);return apiResponse(200,disabledPayload,{'X-Legacy':'1'})}}),disabledResult=await disabled(event);assert.strictEqual(previousCalls,1);assert.strictEqual(parseBody(disabledResult).legacy,true);assert.strictEqual(disabledResult.headers['X-Legacy'],'1');

 previousCalls=0;const cachedPayload=canonicalPayload();let receivedReadEnv=null;
 ...
 const freshPayload=canonicalPayload();
 ...
 const blobsDown=createHandler({...baseDeps,enabled:()=>true,readPublicSnapshot:async()=>{const error=new Error('Blobs unavailable');error.code='BLOBS_EVENT_CONTEXT_MISSING';throw error},writePublicSnapshot:async()=>{writes+=1},previousHandler:async()=>{previousCalls+=1;return apiResponse(200,freshPayload)}}),blobFallback=await blobsDown(event);assert.strictEqual(previousCalls,1);assert.strictEqual(writes,0);assert.strictEqual(blobFallback.headers['X-Public-Snapshot'],'BLOB_UNAVAILABLE');assert.strictEqual(blobFallback.headers['X-Public-Snapshot-Error'],'BLOBS_EVENT_CONTEXT_MISSING');assert.deepStrictEqual(parseBody(blobFallback),freshPayload);

 const invalidPayload=canonicalPayload();delete invalidPayload.propietarios[7].balanceEngineVersion;
 const lastGood=createHandler({...baseDeps,enabled:()=>true,readPublicSnapshot:async()=>snapshot(cachedPayload,Date.now()-1,'etag-last-good'),claimPublicRefresh:async()=>({ok:true,key:'lease',lease:{operationId:'op'}}),releasePublicRefresh:async()=>{},previousHandler:async()=>apiResponse(200,invalidPayload)}),lastGoodResult=await lastGood(event);assert.strictEqual(lastGoodResult.statusCode,200);assert.strictEqual(lastGoodResult.headers['X-Public-Snapshot'],'STALE_FINANCIAL_CONTRACT');assert.strictEqual(lastGoodResult.headers['X-Financial-Contract'],'LAST_GOOD');assert.deepStrictEqual(parseBody(lastGoodResult),cachedPayload);
 const noLastGood=createHandler({...baseDeps,enabled:()=>true,readPublicSnapshot:async()=>({ok:false,reason:'missing'}),claimPublicRefresh:async()=>({ok:true,key:'lease',lease:{operationId:'op'}}),releasePublicRefresh:async()=>{},previousHandler:async()=>apiResponse(200,invalidPayload)}),noLastGoodResult=await noLastGood(event);assert.strictEqual(noLastGoodResult.statusCode,503);assert.strictEqual(noLastGoodResult.headers['X-Public-Snapshot'],'CONTRACT_REJECTED');assert.strictEqual(parseBody(noLastGoodResult).code,'FINANCIAL_CONTRACT_UNAVAILABLE');
 const disabledInvalid=createHandler({...baseDeps,enabled:()=>false,previousHandler:async()=>apiResponse(200,invalidPayload)}),disabledInvalidResult=await disabledInvalid(event);assert.strictEqual(disabledInvalidResult.statusCode,503);assert.strictEqual(disabledInvalidResult.headers['X-Public-Snapshot'],'CONTRACT_REJECTED');assert.strictEqual(parseBody(disabledInvalidResult).message,'Información financiera temporalmente no disponible. El administrador ya fue notificado.');
