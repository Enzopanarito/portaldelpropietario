...
(async()=>{
 ...
 previousCalls=0;const cachedPayload={balanceEngineVersion:PUBLIC_DATA_ENGINE_VERSION,officialBalanceSource:OFFICIAL_BALANCE_SOURCE,propietarios:[]};let receivedReadEnv=null;
 const hit=createHandler({...baseDeps,enabled:()=>true,readPublicSnapshot:async env=>{receivedReadEnv=env;return snapshot(cachedPayload)},previousHandler:async()=>{previousCalls+=1;return apiResponse(500,{})}}),hitResult=await hit(event);assert.strictEqual(previousCalls,0);assert.strictEqual(connections,1);assert.strictEqual(receivedReadEnv,testEnv);assert.strictEqual(hitResult.headers['X-Public-Snapshot'],'HIT');assert.strictEqual(hitResult.headers['X-Balance-Engine'],String(PUBLIC_DATA_ENGINE_VERSION));assert.deepStrictEqual(parseBody(hitResult),cachedPayload);

 let writes=0,releases=0;previousCalls=0;let claimEnv=null,releaseEnv=null;
 const freshPayload={generatedAt:'2026-09-01T22:38:47.886Z',balanceEngineVersion:PUBLIC_DATA_ENGINE_VERSION,officialBalanceSource:OFFICIAL_BALANCE_SOURCE,propietarios:Array.from({length:15},(_,index)=>({Casa:index+1,'Saldo USD Actual':0,'Saldo Bs Ref Actual':0,'Saldo Total Actual':0}))};
