...
test('un comprobante duplicado muestra un mensaje persistente y permite elegir otro archivo',async()=>{
  ...
  try{
    ...
    await page.locator('html[data-vla-owner-payment-report="smart-v5"]').waitFor({state:'attached'});
    ...
    await assert.doesNotReject(()=>validation.getByText('Comprobante ya utilizado',{exact:true}).waitFor({state:'visible',timeout:10000}));
    assert.match(await validation.innerText(),/Este comprobante ya fue utilizado/);
    assert.equal(await page.locator('#submitReport').innerText(),'Enviar reporte');
    ...
    assert.match(await page.locator('#vla-pay-scan').innerText(),/Selecciona otro comprobante/);
    ...
    await page.getByText(/Lectura completada/).waitFor({state:'visible',timeout:10000});
