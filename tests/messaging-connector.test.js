...
const claims={jobId:sim.job.jobId,mode:'Simulación',revision:1};
assert.doesNotThrow(()=>assertTokenMatchesJob(claims,{...sim.job,revision:8}));
assert.throws(()=>assertTokenMatchesJob({...claims,jobId:'WA-OTHER-1234567890'},sim.job),/no corresponde/);
assert.throws(()=>assertTokenMatchesJob({...claims,revision:99},sim.job),/revisión/);
assert.throws(()=>assertTokenMatchesJob(claims,{...sim.job,revision:2},{initial:true}),error=>error instanceof JobConflictError);
