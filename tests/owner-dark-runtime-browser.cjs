...
(async()=>{
  ...
  page.on('console',message=>{const text=message.text();if(message.type()==='error'&&!/cdn\.tailwindcss|app\.netlify\.com|permissions policy|Failed to load resource/i.test(text))errors.push(text)});
  ...
  // Prueba la interacción humana real, no una precondición artificial del navegador.
  await page.locator('#theme1').click();
  await page.waitForFunction(()=>document.documentElement.classList.contains('dark')&&localStorage.getItem('theme')==='dark',null,{timeout:10000});
  ...
  // La preferencia debe sobrevivir una navegación completa.
  ...
  const result={target:TARGET,darkActive:true,persisted:true,houses:15,audits:[welcomeAudit,themeAudit,headerAudit,modalAudit],errors};
