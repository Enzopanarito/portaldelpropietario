...
const recurring=read('admin-recurring-expenses.js');
...
assert(client.includes('Listo para activar'));
assert(client.includes('Bloqueado por dependencias faltantes'));
assert(client.includes('configuración segura'));
assert(recurring.includes('Destino confirmado:'));
for(const filter of ['Activos del mes','Precargados del mes siguiente','Recurrentes / fijos','Eventuales / variables'])assert(recurring.includes(filter));
assert(admin.includes('Saldo neto ref.'));
assert(admin.includes('c.netTotal>0'));
assert(admin.includes('data-vla-expense-state'));
assert(!admin.includes("setupEvents();loadAll(true);loadHealth()"));
assert(audit.includes('Monto histórico en Bs no registrado'));
assert(audit.includes('todavía no hay cortes mensuales registrados'));
...
assert(admin.includes('Pedir información'));
assert(admin.includes('Rechazar verificado'));
