...
(()=>{
 ...
 const grayscale=Array.from({length:72},(_,index)=>index%9);
 const hash=duplicate.dHashFromGrayscale(grayscale);assert.match(hash,/^[a-f0-9]{16}$/);assert.strictEqual(duplicate.hammingDistance(hash,hash),0);
 const oneBit=(BigInt(`0x${hash}`)^1n).toString(16).padStart(16,'0');assert.strictEqual(duplicate.hammingDistance(hash,oneBit),1);
 assert.strictEqual(duplicate.hammingDistance(hash,'bad'),Infinity);

 const exactSha='a'.repeat(64),fingerprint='BANCO|TRANSFER VE|0001|VES|100.00|2026-07-13|ENZO PANARITO';
 const reports=[{id:'recExact',fields:{'Hash SHA-256':exactSha,'Estado':'Rechazado','Casa al Reportar':1}},{id:'recVisual',fields:{'Hash Perceptual':oneBit,'Estado':'Pendiente','Casa al Reportar':2}},{id:'recFingerprint',fields:{'Huella Financiera':fingerprint,'Estado':'Confirmado','Casa al Reportar':3}},{id:'recReferenceOnly',fields:{'Referencia Detectada':'000-999','Banco o Plataforma Detectada':'BANCO A','Método Detectado':'TRANSFER_VE','Moneda Detectada':'VES','Monto Detectado':200,'Fecha Operación Detectada':'2026-07-12','Receptor Detectado':'ENZO PANARITO','Estado':'Pendiente'}}];
 const exact=duplicate.findDuplicateMatches({exactSha,visualHash:hash,fingerprint:'different',reference:'x'},{reports});assert.strictEqual(exact.isDuplicate,true);assert.strictEqual(exact.type,'Hash exacto');assert.strictEqual(exact.strongMatches[0].id,'recExact');
