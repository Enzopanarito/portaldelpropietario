...
<section id='whatsapp-control' class='section'>
  <div class='bg-white rounded-2xl shadow-lg p-5 sm:p-6'>
    ...
    <div class='grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-6'>
      ...
      <div class='bg-slate-100 p-4 rounded-xl'><p class='text-xs text-slate-500'>WhatsApp</p><p id='wa-kpi-session' class='text-xl font-bold'>—</p></div>
    ...
    <div class='grid grid-cols-1 xl:grid-cols-2 gap-6'>
      <div class='border border-slate-200 rounded-2xl p-5'>
        ...
        <p class='text-xs text-slate-500 mb-4'>Los envíos solo pueden comenzar entre 08:00 y 20:59, hora Venezuela. Esta barrera no se puede desactivar desde el Admin.</p>
      ...
      <div class='border border-slate-200 rounded-2xl p-5'>
        ...
        <p class='text-sm text-slate-500 mb-5'>El disparo manual ignora los horarios automáticos, pero nunca la ventana 08:00–21:00 ni la protección contra duplicados.</p>
...
(function(){
  ...
  function render(d){waState=d||{};const cfg=d.config||{},agent=d.agent||{},session=d.session||{},runtime=d.runtime||{};el('wa-kpi-mode').textContent=modeText(cfg.mode);el('wa-kpi-session').textContent=session.loggedIn===true?'🟢 Vinculado':session.loggedIn===false?'🔴 Desconectado':'🟡 Sin comprobar';el('wa-kpi-next').textContent=fmt(runtime.nextRunAt);el('wa-kpi-last').textContent=runtime.lastResult||'—';el('wa-mode').value=cfg.mode||'automatic';el('wa-warmup-minutes').value=Number(cfg.warmupMinutes??5);setTimes(cfg.schedules||['09:00','18:00']);el('wa-last-warmup').textContent=fmt(runtime.lastWarmupAt);el('wa-last-run').textContent=fmt(runtime.lastRunAt);el('wa-last-error').textContent=runtime.lastError||'—';el('wa-history').innerHTML=historyTable(d.history||[]);const ok=d.ok===true&&agent.ok!==false;const banner=el('wa-control-banner');banner.className=(ok?'health-ok':'health-error')+' border-l-4 p-4 rounded mb-5';banner.innerHTML=ok?'<b>✅ Controlador WhatsApp operativo</b><br><span class="text-sm">Modo '+modeText(cfg.mode)+' · agente '+(agent.mode||'—')+'</span>':'<b>❌ Controlador WhatsApp requiere atención</b><br><span class="text-sm">'+(d.message||runtime.lastError||'No disponible')+'</span>'}
  async function load(){try{render(await adminFetch(endpoint))}catch(e){render({ok:false,message:e.message,config:waState&&waState.config||{}})}}
  ...
  async function runNow(){if(!confirm('¿Ejecutar ahora una revisión REAL de recordatorios? Se respetarán saldos actuales, duplicados y la ventana 08:00–21:00.'))return;try{const d=await post('run-now',{confirm:'ENVIAR'});toast(d.message||'Ejecución manual solicitada.');render(d)}catch(e){toast(e.message,true)}}
  ...
  function bind(){if(!el('wa-refresh')||el('wa-refresh').dataset.bound)return;el('wa-refresh').dataset.bound='1';el('wa-refresh').onclick=load;el('wa-warmup').onclick=warmup;el('wa-save-config').onclick=save;el('wa-run-now').onclick=runNow;el('wa-pause').onclick=()=>simple('pause','Automatización pausada.');el('wa-resume').onclick=()=>simple('resume','Automatización reanudada.');el('wa-add-time').onclick=()=>el('wa-times').appendChild(timeRow('09:00'));const nav=document.querySelector("[data-target='whatsapp-control']");if(nav)nav.addEventListener('click',()=>setTimeout(load,0));}
