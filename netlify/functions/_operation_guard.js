...
function prefix(scope, key) {
  return `${PREFIX}${String(scope || '').replace(/[^A-Za-z0-9_-]/g, '_')}|${digest(key)}|`;
}
...
function firstByTime(items) {
  return [...items].sort((a, b) => {
    const x = Number.isFinite(a.createdAt) ? a.createdAt : Number.MAX_SAFE_INTEGER;
    const y = Number.isFinite(b.createdAt) ? b.createdAt : Number.MAX_SAFE_INTEGER;
