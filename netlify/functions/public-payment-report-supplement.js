...
async function handler(event){
 ...
 try{
  ...
  if(!tracking.validRecordId(ownerId)||!tracking.validRecordId(reportId))return json(400,{message:'Código de seguimiento inválido.'});
  ...
  const legacyAuthorized=tracking.TOKEN_PATTERN.test(token)&&tracking.verifyTrackingToken(token,fields['Tracking Token Hash']);
  const sessionAuthorized=Boolean(ownerSession.sessionFromEvent(event,ownerId));
  if(!tracking.ownerMatches(fields,ownerId)||(!legacyAuthorized&&!sessionAuthorized))return json(404,{message:'No se encontró un reporte asociado a esta autorización.'});
  ...
  const patch={'Respuesta del Propietario':appendResponse(fields['Respuesta del Propietario'],responseText,now),'Fecha Respuesta Propietario':now,'Estado de Procesamiento':'Pendiente de administrador','Decisión Administrativa':'Pendiente','Notificación Propietario':'Información recibida. Administración continuará la revisión.','Log de Auditoría':appendAudit(fields['Log de Auditoría'],{action:'owner-supplement',result:'information-received',at:now,attachment:Boolean(attachment),authorization:sessionAuthorized?'verified-device':'tracking-token'})};
