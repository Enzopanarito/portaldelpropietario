...
function createProofStore({storeFactory=defaultStore,encryptionKey,encryptionKeys,now=()=>new Date()}={}){
 ...
 async function getByKey({key:storedKey,attachmentSha,contentType,variant='original'},env=process.env){
  const key=validateStoredKey(storedKey,env),store=await storeFactory(),entry=await store.getWithMetadata(key,{type:'arrayBuffer',consistency:'strong'});if(!entry)return null;const metadata=entry.metadata||{},expectedSha=clean(attachmentSha||metadata.sha256).toLowerCase(),normalizedType=clean(contentType||metadata.contentType).toLowerCase();if(!/^[a-f0-9]{64}$/.test(expectedSha))throw codedError('El hash del comprobante almacenado no es válido.','PROOF_HASH_MISMATCH');if(metadata.sha256&&metadata.sha256!==expectedSha)throw codedError('Los metadatos del comprobante no coinciden con su clave.','PROOF_STORE_COLLISION');if(metadata.variant&&metadata.variant!==variant)throw codedError('La variante almacenada no coincide con la solicitada.','PROOF_STORE_COLLISION');if(metadata.contentType&&metadata.contentType!==normalizedType)throw codedError('El MIME almacenado no coincide con el solicitado.','PROOF_STORE_COLLISION');
 ...
 async function reserveIdentity({attachmentSha,requestId,ownerId,ttlMs=15*60*1000},env=process.env){
  ...
  const existing=await store.getWithMetadata(key,{type:'json',consistency:'strong'}),data=existing?.data||{};if(data.requestId===request&&data.ownerScope===payload.ownerScope){if(data.state==='COMPLETED'&&clean(data.reportId))return{acquired:false,duplicate:false,idempotent:true,key,requestId:request,state:data.state,reportId:clean(data.reportId)};return{acquired:true,created:false,resumed:true,key,etag:existing.etag||result.etag||'',requestId:request,state:data.state||'RESERVED',reportId:clean(data.reportId)}}
