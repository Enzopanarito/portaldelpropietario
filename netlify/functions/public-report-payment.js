...
async function loadRecentReports(){
  ...
  ['Propietario que Reporta','Forma de Pago Reportada','Equivalente USD Reportado','Monto Reportado','Referencia','Observaciones Reportadas'].forEach(field=>params.append('fields[]',field));
...
function reportHasSubmission(report,submissionId){return String(report?.fields?.['Observaciones Reportadas']||'').split(/\r?\n/).some(line=>line.trim()===`ID de envío: ${submissionId}`)}
function idempotentReportResponse(reportId,trackingCredential){return json(200,{success:true,idempotent:true,reportId,trackingCode:paymentTracking.trackingCode(reportId,trackingCredential.token),message:'Este reporte ya había sido recibido correctamente. No se creó un duplicado.'})}
function duplicateReportResponse(message='Este pago ya fue reportado o está siendo procesado. No se creó otro reporte.'){return json(409,{success:false,duplicate:true,duplicateLevel:'confirmed',duplicateScore:100,duplicateType:'Identidad financiera del reporte',duplicateEvidence:['propietario, referencia, monto, moneda y fecha coincidentes'],canSubmitForReview:false,message})}
...
const handler = async function(event){
  ...
  try{
    ...
    const trackingCredential=paymentTracking.createTrackingCredential(ownerId,submissionId);
    ...
    const zelleNoReference=paymentChannel==='DIGITAL'&&method==='ZELLE'&&!rawReference&&String(recipientVerification?.classification||'').toUpperCase()==='CONFIRMED';
    ...
    const reference=rawReference||(paymentChannel==='CASH'?`EFECTIVO · ${cashReceiver} · ${submissionId.slice(-12)}`:(zelleNoReference?`ZELLE-SIN-REF-${attachmentSha.slice(0,16).toUpperCase()}`:''));
    const presentedWarnings=uncertaintyWarnings({paymentChannel,recipientVerification});
    ...
    const recentDuplicate=paymentChannel==='DIGITAL'?await findRecentDuplicate({ownerId,mode,amountUsdRef:usdEq,reference}):null;
    if(recentDuplicate){
      if(validRecordId(recentDuplicate.id)&&reportHasSubmission(recentDuplicate,submissionId))return idempotentReportResponse(recentDuplicate.id,trackingCredential);
      return duplicateReportResponse('Este pago ya tiene un reporte reciente con la misma referencia, cuenta y monto. No se creó otro reporte.');
    }

    let proof=null,identity=null,identityReservation=null,financialReservation=null,proofStore=null,dedupStore=null,historicalDuplicate={isDuplicate:false,possibleDuplicate:false,level:'none',score:0,type:'Sin coincidencia',evidence:[],matches:[]};
    if(paymentChannel==='DIGITAL'){
      ...
      if(historicalDuplicate.isDuplicate)return json(409,{success:false,duplicate:true,duplicateLevel:'confirmed',duplicateScore:historicalDuplicate.score||100,duplicateType:historicalDuplicate.type,duplicateEvidence:historicalDuplicate.evidence||[],canSubmitForReview:false,message:'Este comprobante o esta transacción ya fue reportado. No se creó ningún reporte adicional.'});
      ...
      dedupStore=createPaymentReportDedupStore();
      const financialIdentity=paymentReportIdentityHash({ownerId,enteredCurrency,amount,reference,transactionDate});
      financialReservation=await dedupStore.reserve({identity:financialIdentity,requestId:submissionId,ownerId});
      if(financialReservation.idempotent)return idempotentReportResponse(financialReservation.reportId,trackingCredential);
      if(!financialReservation.acquired)return duplicateReportResponse(financialReservation.pending?'Este pago ya está siendo recibido. No se creó un segundo reporte.':'Este pago ya fue reportado. No se creó otro reporte.');
      identityReservation=await proofStore.reserveIdentity({attachmentSha:identity.sha256,requestId:submissionId,ownerId});
      if(identityReservation.idempotent)return idempotentReportResponse(identityReservation.reportId,trackingCredential);
      if(!identityReservation.acquired)return json(409,{success:false,duplicate:true,duplicateLevel:'confirmed',duplicateScore:100,duplicateType:'Hash exacto',duplicateEvidence:['archivo SHA-256 idéntico'],canSubmitForReview:false,message:'Este comprobante ya está siendo procesado o fue usado anteriormente. No se creó un reporte nuevo.'});
      proof=await storeEncryptedProof(`upload-${identity.sha256}`,attachment,identity,proofStore);
    }

    const possibleDuplicate=historicalDuplicate.possibleDuplicate===true;
    const reportContext=[`Canal reportado: ${paymentChannel==='CASH'?'EFECTIVO':'DIGITAL'}`,paymentChannel==='CASH'?`Efectivo entregado a: ${cashReceiver}`:'',`Método detectado/confirmado: ${method}`,zelleNoReference?'Referencia visible: Zelle no muestra una referencia en esta confirmación; se usa una identidad interna derivada del comprobante.':'',`Fecha de operación: ${transactionDate||'NO DETECTADA'}`,`Fuente de fecha: ${transactionDateSource} (${dateSourceLabel(transactionDateSource,paymentChannel)})`,`Confianza de fecha: ${transactionDateConfidence}`,`Fecha requiere contraste: ${transactionDateNeedsReview?'SÍ':'NO'}`,`Evidencia de fecha: ${transactionDateEvidence}`,`Estado interno inicial: ${transactionStatus}`,presentedWarnings.length?`Alertas aceptadas por el propietario: ${presentedWarnings.join(' · ')}`:'',possibleDuplicate?`Alerta no bloqueante de similitud: ${historicalDuplicate.type}.`:'',paymentChannel==='DIGITAL'?`Prelectura completa: ${analysisSummary.prefillComplete?'SÍ':'NO'}`:'',analysisSummary.provider?`Proveedor/modelo de prelectura: ${analysisSummary.provider}`:'',analysisSummary.route?`Ruta de prelectura: ${analysisSummary.route}`:'',paymentChannel==='DIGITAL'?`Confianza de prelectura: ${Math.round(analysisSummary.confidence*100)}%`:'',analysisSummary.transactionTime?`Hora visible detectada: ${analysisSummary.transactionTime}`:'',analysisSummary.transactionStatus?`Estado visible detectado: ${analysisSummary.transactionStatus}`:'',analysisSummary.recipient?`Receptor visible detectado: ${analysisSummary.recipient}`:'',paymentChannel==='DIGITAL'?`Posible modificación visual: ${analysisSummary.possibleVisualModification?'SÍ':'NO señalada'}`:'',analysisSummary.warnings.length?`Advertencias de prelectura: ${analysisSummary.warnings.join(' · ')}`:'',analysisSummary.missingLabels.length?`Datos que requirieron apoyo: ${analysisSummary.missingLabels.join(' · ')}`:'',`ID de envío: ${submissionId}`,observations].filter(Boolean).join('\n');
    const fields={'Propietario que Reporta':[ownerId],'Monto Reportado':usdEq,Referencia:reference,Estado:'Pendiente','Fecha del Reporte':reportDate,'Forma de Pago Reportada':mode,'Equivalente USD Reportado':usdEq,'Estado Acceso al Reportar':String(ownerFields['Estado Acceso Portón']||'Sin configurar'),'Casa al Reportar':Number(ownerFields.Casa||0),'Fecha y Hora del Reporte':reportTimestamp,'Moneda Ingresada':enteredCurrency==='BS'?'VES':'USD','Monto Ingresado':amount,'Fuente Tasa BCV Reporte':rateInfo.source,'Archivo Obligatorio':paymentChannel==='DIGITAL','Estado de Procesamiento':paymentChannel==='CASH'?'Pendiente de administrador':'Recibido','Resultado Validación':paymentChannel==='CASH'?'Revisión manual urgente':'Pendiente','Decisión Administrativa':'Pendiente','Banco Reportado':bank,'Observaciones Reportadas':reportContext,'Fuente Fecha Operación':transactionDateSource,'Confianza Fecha Operación':transactionDateConfidence,'Fecha Requiere Revisión':transactionDateNeedsReview,'Evidencia Fecha Operación':transactionDateEvidence,'Posible Duplicado':possibleDuplicate,'Nivel de Duplicado':possibleDuplicate?'possible':'none','Puntaje de Duplicado':possibleDuplicate?(historicalDuplicate.score||0):0,'Tipo de Coincidencia':possibleDuplicate?historicalDuplicate.type:'Sin coincidencia','Evidencia de Duplicado':JSON.stringify(possibleDuplicate?(historicalDuplicate.evidence||[]):[]),'Detalle de Coincidencia':JSON.stringify(historicalDuplicate.matches||[]),'Tracking Token Hash':trackingCredential.hash,'Alerta Aceptada por Propietario':Boolean(presentedWarnings.length&&uncertaintyAcknowledged),'Alertas Presentadas':presentedWarnings.join('\n')};
    ...
    if(paymentChannel==='DIGITAL'){
      Object.assign(fields,{
        ...
        'Referencia Detectada':rawReference,
    ...
    if(financialReservation)await dedupStore.complete({reservation:financialReservation,reportId:report.id}).catch(error=>console.warn('No se pudo completar la barrera financiera idempotente:',error.message));
    ...
    const requiresReview=presentedWarnings.length>0||transactionDateNeedsReview||analysisSummary.possibleVisualModification||!analysisSummary.prefillComplete||!['COMPLETED','SENT','PROCESSED'].includes(analysisSummary.transactionStatus);
    ...
    return json(200,deepEscapeStrings({success:true,message,reportId:report?.id,trackingCode:paymentTracking.trackingCode(report.id,trackingCredential.token),paymentChannel,targetMode:mode,enteredCurrency,amountEntered:amount,amountUsdRef:usdEq,amountBs,rateApplied:rateInfo.rate||null,method,transactionDate,transactionDateSource,transactionDateConfidence,transactionDateNeedsReview,transactionDateEvidence,uncertaintyAcknowledged:presentedWarnings.length?uncertaintyAcknowledged:false,uncertaintyWarnings:presentedWarnings,duplicateReview:false,reviewDeadlineHours:requiresReview?72:null,reportTimestamp,attachmentIncluded:Boolean(attachment),proofStored:Boolean(proof?.verified),visualHashStored:Boolean(identity?.visualHash),automation,access,adminNotification}));
  }catch(error){
...
exports.reportHasSubmission = reportHasSubmission;
exports.normalizePaymentMethod = normalizePaymentMethod;
exports.normalizeAnalysisSummary = normalizeAnalysisSummary;
exports.uncertaintyWarnings = uncertaintyWarnings;
exports.paymentReportIdentityHash = paymentReportIdentityHash;
