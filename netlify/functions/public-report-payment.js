...
const handler = async function(event){
  ...
  try{
    ...
    if(paymentChannel==='DIGITAL'){
      ...
      if(identityReservation.idempotent)return json(200,{success:true,idempotent:true,reportId:identityReservation.reportId,trackingCode:paymentTracking.trackingCode(identityReservation.reportId,trackingCredential.token),message:'Este reporte ya había sido recibido correctamente. No se creó un duplicado.'});
    ...
    const message=paymentChannel==='CASH'?'Efectivo reportado. Quedó pendiente de confirmación administrativa; no se modificará el saldo ni el acceso hasta que la entrega sea verificada.':'Pago recibido. Ya puedes cerrar esta pantalla: aparecerá En proceso mientras el sistema lo aprueba automáticamente o lo envía a revisión. Mientras se valida, no cambia tu saldo ni tu acceso.';
