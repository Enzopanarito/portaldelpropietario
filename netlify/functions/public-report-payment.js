...
const { todayCaracasISO, timestampCaracasISO, resolveSubmittedDate } = require('./_shared/_payment_date_resolver');
...
const handler = async function(event){
  ...
  try{
    ...
    const proofExtractedVerified=paymentChannel==='DIGITAL'&&verifyDateAttestation(body.transactionDateAttestation,{ownerId,method,transactionDate:clientTransactionDate,transactionDateSource:body.transactionDateSource,attachmentContent:attachment?.content});
    const dateResolution=resolveSubmittedDate({clientDate:clientTransactionDate,clientSource:body.transactionDateSource,attachment:body.attachment,paymentChannel,method,proofExtractedVerified}),{transactionDate,transactionDateSource,transactionDateConfidence,transactionDateNeedsReview,transactionDateEvidence}=dateResolution;
    ...
    if(paymentChannel==='DIGITAL'){
      Object.assign(fields,{
        'Normalized Analysis JSON':JSON.stringify({stage:'prefill',untrustedClientRelay:true,method,reportTimestamp,reportTimestampCaracas,timeZone:'America/Caracas',provider:analysisSummary.provider,route:analysisSummary.route,confidence:analysisSummary.confidence,transactionDate,transactionDateSource,transactionDateConfidence,transactionDateNeedsReview,transactionDateEvidence,proofExtractedVerified,clientDateSourceClaim:optionalText(body.transactionDateSource,40),fileLastModifiedAuxiliary:body.attachment?.lastModified||body.attachment?.lastModifiedISO||null,transactionTime:analysisSummary.transactionTime,transactionStatus:analysisSummary.transactionStatus,recipient:analysisSummary.recipient,warnings:analysisSummary.warnings,possibleVisualModification:analysisSummary.possibleVisualModification,prefillComplete:analysisSummary.prefillComplete,missingLabels:analysisSummary.missingLabels}),
    ...
    console.info(JSON.stringify({event:'VLA_PAYMENT_REPORT_CREATED',reportId:report?.id||null,ownerId,paymentChannel,method,submissionId,reportTimestamp,reportTimestampCaracas,transactionDateSource,proofExtractedVerified,targetMode:mode,proofStored:Boolean(proof?.verified),analysisQueued:Boolean(automation.queued)}));
    return json(200,deepEscapeStrings({success:true,message,reportId:report?.id,paymentChannel,targetMode:mode,enteredCurrency,amountEntered:amount,amountUsdRef:usdEq,amountBs,rateApplied:rateInfo.rate||null,method,transactionDate,transactionDateSource,transactionDateConfidence,transactionDateNeedsReview,transactionDateEvidence,reportTimestamp,reportTimestampCaracas,proofExtractedVerified,attachmentIncluded:Boolean(attachment),proofStored:Boolean(proof?.verified),visualHashStored:Boolean(identity?.visualHash),automation,access,adminNotification}));
  }catch(error){
