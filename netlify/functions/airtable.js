...
function usageSourceFromPath(path,method){ const decoded=decodeURIComponent(path||'').replace(/^\//,'').replace(/\//g,'-').toLowerCase()||'unknown'; return `proxy-${method.toLowerCase()}-${decoded}`.slice(0,80); }
exports.handler=async function(event){
  ...
  try{
    ...
    airtableCalls+=1; const response=await fetch(url,{method:httpMethod,headers:{Authorization:`Bearer ${AIRTABLE_API_TOKEN}`,'Content-Type':'application/json'},body:httpMethod!=='GET'?body:undefined}); const data=await response.json(); await recordApiUsage(usageSourceFromPath(airtablePath,httpMethod),airtableCalls,AIRTABLE_API_TOKEN,AIRTABLE_BASE_ID);
    if(!response.ok) return {statusCode:response.status,headers:{'Content-Type':'application/json','X-Airtable-Calls':String(airtableCalls+1)},body:JSON.stringify(data)};
    if(httpMethod==='GET') cache.set(cacheKey,{data,expiresAt:Date.now()+getCacheTtl(airtablePath)}); else clearCache();
    return {statusCode:200,headers:{'Content-Type':'application/json','X-Cache':httpMethod==='GET'?'MISS':'BYPASS','X-Airtable-Calls':String(airtableCalls+1),'Cache-Control':httpMethod==='GET'?'private, max-age=60':'no-store'},body:JSON.stringify(data)};
  }catch(error){ await recordApiUsage(usageSourceFromPath(airtablePath,httpMethod)+'-error',airtableCalls,AIRTABLE_API_TOKEN,AIRTABLE_BASE_ID); return {statusCode:500,headers:{'Content-Type':'application/json','X-Airtable-Calls':String(airtableCalls)},body:JSON.stringify({message:'Error en la función del servidor.',detail:error.message})}; }
};
