...
async function waitForSnapshot(readSnapshot=snapshotStore.readPublicSnapshot,sleepFn=sleep){for(let attempt=0;attempt<12;attempt+=1){await sleepFn(250);const current=await readSnapshot().catch(()=>null);if(current&&current.ok&&current.fresh)return current}return null}
...
function createHandler(deps={}){
 ...
 return async function handler(event){
  ...
  let lease=null;
  if(!blobReadError){try{lease=await claimRefresh()}catch(error){blobReadError=error}}
  if(lease&&!lease.ok){
  ...
  try{
   ...
   if(fresh.statusCode===200){
    ...
    if(!blobReadError){try{await writeSnapshot(payload,undefined,versionRead)}catch(error){writeWarning=`${error.code||'PUBLIC_SNAPSHOT_WRITE'}: ${String(error.message||'').slice(0,240)}`}}
  ...
  }finally{if(lease&&lease.ok)await releaseRefresh(lease).catch(()=>null)}
 };
}
