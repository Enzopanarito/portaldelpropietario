...
const {currentMonthCaracas}=require('./_shared/_expense_lifecycle');
...
function isWhatsappFinancialAgent(event){return/VLA-WhatsApp-Agent\//i.test(requestHeader(event,'user-agent'))}
function whatsappFinancialPaused(event,env=process.env){return String(env?.VLA_WHATSAPP_FINANCIAL_PAUSE||'').trim().toLowerCase()==='true'&&isWhatsappFinancialAgent(event)}
function snapshotMatchesAccountingMonth(snapshot,month=currentMonthCaracas()){return String(snapshot?.payload?.accountingMonth||'')===String(month||'')}
async function waitForSnapshot(readSnapshot=snapshotStore.readPublicSnapshot,sleepFn=sleep,env=process.env){for(let attempt=0;attempt<12;attempt+=1){await sleepFn(250);const current=await readSnapshot(env).catch(()=>null);if(current&&current.ok&&current.fresh&&snapshotMatchesAccountingMonth(current.snapshot))return current}return null}
...
function createHandler(deps={}){
 ...
 const isFinancialAgent=deps.isWhatsappFinancialAgent||isWhatsappFinancialAgent;
 const snapshotMonthMatches=deps.snapshotMatchesAccountingMonth||snapshotMatchesAccountingMonth;

 return async function handler(event){
  const host=eventHost(event);
  const snapshotEnv=eventEnvironment(event);
  const accountingMonth=currentMonthCaracas(now());
  ...
  // Cortacircuito financiero manual. Se conserva como protección operacional
  // independiente incluso después de automatizar el bloqueo de frontera mensual.
  ...
  const cachedSameMonth=cached&&cached.ok&&snapshotMonthMatches(cached.snapshot,accountingMonth);
  if(cachedSameMonth&&cached.fresh)return cachedResponse(cached.snapshot,'HIT');
  ...
  if(lease&&!lease.ok){
   // Un propietario puede ver la última fotografía certificada mientras se
   // revalida. El agente financiero de WhatsApp, en cambio, falla cerrado:
   // nunca envía una fotografía vieja o de otro mes.
   if(cached&&cached.ok&&!isFinancialAgent(event))return cachedResponse(cached.snapshot,'STALE',{'Warning':'110 - "Respuesta pública temporalmente antigua durante revalidación"','X-VLA-Accounting-Month':String(cached.snapshot?.payload?.accountingMonth||'unknown')});
  ...
  try{
   ...
   if(fresh.statusCode===200){
    // Defensa en profundidad: una respuesta nueva debe declarar el mes contable
    // que está sirviendo. Así un snapshot de agosto jamás puede hacerse pasar por septiembre.
    if(String(payload.accountingMonth||'')!==accountingMonth){
     return response(503,{code:'ACCOUNTING_MONTH_MISMATCH',message:'La respuesta financiera no corresponde al mes contable actual.'},{'Retry-After':'30','X-VLA-Accounting-Transition':'1'});
    }
   ...
   const accountingBlocked=payload?.code==='ACCOUNTING_TRANSITION_PENDING';
   if(accountingBlocked&&isFinancialAgent(event)){
    return response(503,payload,{...(fresh.headers||{}),'Retry-After':'60','X-VLA-Accounting-Transition':'1','X-Public-Snapshot':'FINANCIAL_FAIL_CLOSED'});
   }
   if(cached&&cached.ok&&!isFinancialAgent(event))return cachedResponse(cached.snapshot,accountingBlocked?'ACCOUNTING_TRANSITION_STALE':'STALE_FALLBACK',{'Warning':accountingBlocked?'112 - "Cierre mensual en proceso; se sirvió la última fotografía certificada"':'111 - "Airtable no disponible; se sirvió la última fotografía validada"',...(accountingBlocked?{'X-VLA-Accounting-Transition':'1'}:{})});
   return fresh;
  }catch(error){
   if(cached&&cached.ok&&!isFinancialAgent(event))return cachedResponse(cached.snapshot,'STALE_EXCEPTION',{'Warning':'111 - "Error de revalidación; se sirvió la última fotografía validada"'});
...
module.exports={handler,createHandler,response,parseBody,forceEvent,cachedResponse,waitForSnapshot,blobErrorCode,requestHeader,isWhatsappFinancialAgent,whatsappFinancialPaused,snapshotMatchesAccountingMonth};
