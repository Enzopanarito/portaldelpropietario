...
function decodeBase64Strict(value,maxBytes=DEFAULT_MAX_BYTES){
 ...
 if(!base64)throw codedError('El comprobante está vacío.','EMPTY_ATTACHMENT');
 if(base64.length>Math.ceil(maxBytes/3)*4+4)throw codedError('El comprobante supera el tamaño permitido.','ATTACHMENT_TOO_LARGE');
 if(!/^[A-Za-z0-9+/]+={0,2}$/.test(base64)||base64.length%4===1)throw codedError('El comprobante contiene una codificación inválida.','INVALID_BASE64');
 const buffer=Buffer.from(base64,'base64');
 if(!buffer.length)throw codedError('El comprobante está vacío.','EMPTY_ATTACHMENT');
 const canonical=buffer.toString('base64').replace(/=+$/,''),provided=base64.replace(/=+$/,'');
 if(canonical!==provided)throw codedError('El comprobante contiene una codificación base64 no canónica.','INVALID_BASE64');
 if(buffer.length>maxBytes)throw codedError('El comprobante supera el tamaño permitido.','ATTACHMENT_TOO_LARGE');
 return buffer;
}
function isoBrands(buffer){
 if(buffer.length<16||buffer.subarray(4,8).toString('ascii')!=='ftyp')return[];
 const declared=buffer.readUInt32BE(0),end=Math.min(buffer.length,declared>=16?declared:buffer.length),brands=[buffer.subarray(8,12).toString('ascii')];
 for(let offset=16;offset+4<=end;offset+=4)brands.push(buffer.subarray(offset,offset+4).toString('ascii'));
 return brands;
}
function detectFormat(buffer){
 ...
 const brands=isoBrands(buffer);if(brands.some(brand=>HEIC_BRANDS.has(brand)))return'heic';if(brands.some(brand=>HEIF_BRANDS.has(brand)))return'heif';
 return null;
}
...
function isTruncated(buffer,format){
 ...
 if(format==='webp'){if(buffer.length<20)return true;const declared=buffer.readUInt32LE(4)+8;return declared>buffer.length}
 if(['heic','heif'].includes(format)){const declared=buffer.length>=4?buffer.readUInt32BE(0):0;return buffer.length<16||declared>buffer.length}
 return false;
}
...
function decodeProofInput(input,{maxBytes=DEFAULT_MAX_BYTES}={}){
 if(!input||typeof input!=='object')throw codedError('El comprobante adjunto no es válido.','INVALID_ATTACHMENT');
 const declaredType=clean(input.type).toLowerCase();if(!ALLOWED_TYPES.has(declaredType))throw codedError('El comprobante debe ser JPG, PNG, WebP, HEIC o PDF.','UNSUPPORTED_ATTACHMENT_TYPE');
 const content=Buffer.isBuffer(input.content)?Buffer.from(input.content):decodeBase64Strict(input.base64,maxBytes);
 if(!content.length)throw codedError('El comprobante está vacío.','EMPTY_ATTACHMENT');
 if(content.length>maxBytes)throw codedError('El comprobante supera el tamaño permitido.','ATTACHMENT_TOO_LARGE');
 const format=detectFormat(content),expected=expectedFormat(declaredType);if(!format||format!==expected)throw codedError('El contenido del comprobante no coincide con el formato declarado.','MIME_SIGNATURE_MISMATCH',{detectedFormat:format,expectedFormat:expected});
...
function buildIdempotencyKey(reportId,attachmentSha,promptVersion){for(const[name,value]of Object.entries({reportId,attachmentSha,promptVersion}))if(!clean(value))throw new Error(`Falta ${name}.`);if(!/^[a-f0-9]{64}$/i.test(clean(attachmentSha)))throw new Error('attachmentSha no es válido.');return`${clean(reportId)}|${clean(attachmentSha).toLowerCase()}|${clean(promptVersion)}`}

module.exports={DEFAULT_MAX_BYTES,MIN_HARD_DIMENSION,MIN_RECOMMENDED_DIMENSION,ALLOWED_TYPES,FORMAT_BY_TYPE,HEIC_BRANDS,HEIF_BRANDS,clean,codedError,sha256,safeFilename,decodeBase64Strict,isoBrands,detectFormat,expectedFormat,isTruncated,pngDimensions,jpegDimensions,webpDimensions,imageDimensions,preliminaryQuality,decodeProofInput,buildIdempotencyKey};
