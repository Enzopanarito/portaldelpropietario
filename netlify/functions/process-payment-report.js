...
const handler=async function(event){
 ...
 try{
  ...
  const patched=await airtablePatchRecord(TABLES.reportes,reportId,{Estado:'Confirmado','Estado de Procesamiento':'Aprobado','Resultado Validación':'Coincidencia exacta verificada','Decisión Administrativa':finalDecision,'Validación Realizada Por':decisionSource==='automatic'?'Motor determinístico':'Administrador','Administrador que Revisó':who,'Fecha Revisión':reviewedAt,'Pago Definitivo Creado':true,'Pago Definitivo Relacionado':[paymentId],...(input.action==='approve_exception'?{'Motivo de Excepción':input.reason}:{}),'Log de Auditoría':audit(fields,{action:input.action,adminId:who,reason:input.reason,corrections:input.corrections,result:paymentCreated?'payment-created':'payment-reused-idempotently',paymentId,at:reviewedAt})});
  writeStage=2;
