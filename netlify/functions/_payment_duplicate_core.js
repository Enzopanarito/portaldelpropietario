...
function normalizeCurrency(value){const text=normalizeText(value);if(['VES','BS','BS BCV','BOLIVARES'].includes(text))return'VES';if(['USD','US DOLLAR','DOLLAR','DOLARES'].includes(text))return'USD';return text||'UNKNOWN'}
...
function candidateFromRecord(record,{kind='report'}={}){const fields=recordFields(record);return{id:clean(record&&record.id),kind,house:Number(fields['Casa al Reportar']||fields.Casa||0)||null,status:clean(fields.Estado||fields['Decisión Administrativa']),exactSha:clean(fields['Hash SHA-256']).toLowerCase(),visualHash:normalizeVisualHash(fields['Hash Perceptual']),fingerprint:clean(fields['Huella Financiera']),reference:normalizeReference(fields['Referencia Detectada']||fields.Referencia),bank:normalizeText(fields['Banco o Plataforma Detectada']||fields['Método de Pago']||''),method:normalizeText(fields['Método Detectado']||fields['Forma de Pago']||fields['Forma de Pago Reportada']||''),currency:normalizeCurrency(fields['Moneda Detectada']||fields['Forma de Pago']||fields['Forma de Pago Reportada']||''),amount:normalizeAmount(fields['Monto Detectado']||fields['Equivalente USD Aplicado']||fields['Monto Pagado']||fields['Equivalente USD Reportado']||fields['Monto Reportado']),date:normalizeDate(fields['Fecha Operación Detectada']||fields['Fecha de Pago']||fields['Fecha del Reporte']),recipient:normalizeText(fields['Receptor Detectado']||'')};}
...
function findDuplicateMatches(input,{reports=[],payments=[],history=[],visualDistance=DEFAULT_VISUAL_DISTANCE,excludeIds=[]}={}){
 const excluded=new Set((excludeIds||[]).map(clean)),needle={exactSha:clean(input.exactSha||input.sha256).toLowerCase(),visualHash:normalizeVisualHash(input.visualHash||input.perceptualHash),fingerprint:clean(input.fingerprint||input.financialFingerprint),reference:normalizeReference(input.reference),bank:normalizeText(input.bank_or_platform||input.bank),method:normalizeText(input.method),currency:normalizeCurrency(input.currency),amount:normalizeAmount(input.amount),date:normalizeDate(input.transaction_date||input.date),recipient:normalizeText(input.recipient_name||input.recipient_phone||input.recipient_email||input.recipient)};
...
module.exports={DEFAULT_VISUAL_DISTANCE,clean,normalizeText,normalizeReference,normalizeDate,normalizeCurrency,normalizeAmount,canonicalFingerprint,fingerprintHash,bitStringToHex,dHashFromGrayscale,normalizeVisualHash,hammingDistance,recordFields,candidateFromRecord,sameReferenceContext,findDuplicateMatches};
