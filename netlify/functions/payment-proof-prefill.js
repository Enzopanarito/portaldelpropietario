...
const {DATE_SOURCES,resolvePrefillDate}=require('./_shared/_payment_date_resolver');
const {createDateAttestation}=require('./_shared/_payment_date_attestation');
...
const handler=async event=>{
 ...
 try{
  ...
  const analysis=contract.normalizeAnalysis(parsed.value),missing=missingFields(analysis),bank=analysis.bank_or_platform||methodLabel(analysis.method),date=resolvePrefillDate({proofDate:analysis.transaction_date,attachment:body.attachment,method:analysis.method});
  const transactionDateAttestation=date.transactionDateSource===DATE_SOURCES.PROOF_EXTRACTED?createDateAttestation({ownerId,method:analysis.method,transactionDate:date.transactionDate,transactionDateSource:date.transactionDateSource,attachmentContent:attachment.content}):'';
  return json(200,{success:true,complete:missing.length===0,analysis:{amount:analysis.amount,currency:analysis.currency,reference:analysis.reference||'',bank,method:analysis.method,...date,transactionDateAttestation,transactionTime:analysis.transaction_time||'',transactionStatus:analysis.transaction_status,recipient:analysis.recipient_name||analysis.recipient_phone||analysis.recipient_email||analysis.recipient_account_visible||'',confidence:analysis.confidence,warnings:analysis.warnings||[],possibleVisualModification:analysis.possible_visual_modification===true},missing,analysisProvider:result.model,analysisRoute:result.provider||'unknown'},{'X-Payment-AI-Provider':result.provider||'unknown'});
 }catch(error){
