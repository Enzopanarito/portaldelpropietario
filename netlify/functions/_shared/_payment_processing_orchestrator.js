...
function createOrchestrator(deps={}){
 ...
 async function run(input={},env=process.env){
  ...
  try{
   ...
   let analysisResult=null,rawPrimary='',rawSecondary='',aiAudit=[],secondaryAttempts=0,resolutionSource='none';
   if(!config.aiEnabled||!config.primaryModel||typeof analysisRunner!=='function')analysisResult={ok:false,reason:'AI_NOT_CONFIGURED',raw:''};
    ...
    for(let guard=0;guard<4;guard+=1){
     ...
     try{
      ...
      if(!secondary&&evaluated.ok){analysisResult=evaluated;resolutionSource=action.action==='PRIMARY_RETRY'?'primary-retry':'primary';break}
      ...
      if(secondary&&dateRefinement&&primaryExtracted){
       if(evaluated.normalized){const merged={...primaryExtracted,transaction_date:evaluated.normalized.transaction_date||null,transaction_time:evaluated.normalized.transaction_time||primaryExtracted.transaction_time,confidence:Math.min(Number(primaryExtracted.confidence)||0,Number(evaluated.normalized.confidence)||0),warnings:[...new Set([...(primaryExtracted.warnings||[]),...(evaluated.normalized.warnings||[])])].slice(0,30)};merged.critical_fields_visible=Boolean(merged.amount&&merged.currency!=='UNKNOWN'&&merged.transaction_date&&merged.reference&&merged.transaction_status!=='UNKNOWN'&&(merged.recipient_name||merged.recipient_phone||merged.recipient_email||merged.recipient_account_visible||merged.recipient_binance_id));analysisResult={...aiContract.evaluateRawOutput(JSON.stringify(merged),{minimumConfidence:config.minimumConfidence}),dateSecondaryUsed:true};resolutionSource='secondary-date';}else{analysisResult={...analysisResult,normalized:primaryExtracted,dateSecondaryUsed:true};resolutionSource='primary-partial'}
       break;
      }
      if(evaluated.ok){analysisResult=evaluated;resolutionSource='secondary';break}
   ...
   failureAnalysis={ok:analysisResult?.ok===true,normalized:analysisResult?.normalized||null,rawPrimary,rawSecondary,audit:aiAudit,dateSecondaryUsed:analysisResult?.dateSecondaryUsed===true,failureReason:analysisResult&&analysisResult.ok?'':clean(analysisResult?.reason),resolutionSource};
