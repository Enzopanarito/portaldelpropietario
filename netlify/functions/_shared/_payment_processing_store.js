...
function createProcessingStore({storeFactory=defaultStore,now=()=>Date.now(),randomBytes=crypto.randomBytes,leaseMs=DEFAULT_LEASE_MS,readDelaysMs=EVENTUAL_READ_DELAYS_MS,sleep=ms=>new Promise(resolve=>setTimeout(resolve,ms))}={}){
 ...
 async function readEntry(store,key){
  let entry=null;
  for(const delay of readDelaysMs){
   if(delay>0)await sleep(delay);
   entry=normalizeEntry(await store.getWithMetadata(key,{type:'json',consistency:undefined}));
 ...
 async function acquire({reportId,idempotencyKey,payloadHash},env=process.env){
  const identity=validateIdentity({idempotencyKey,payloadHash}),key=processingKey(reportId,env),store=await storeFactory(),existing=await readEntry(store,key);
 ...
 async function read(reportId,env=process.env){const key=processingKey(reportId,env),store=await storeFactory();return readEntry(store,key)}
 return{acquire,update,complete,fail,read};
}

module.exports={STORE_NAME,SCHEMA_VERSION,DEFAULT_LEASE_MS,EVENTUAL_READ_DELAYS_MS,ALLOWED_ENVIRONMENTS,clean,sha256,codedError,environmentName,airtableBaseId,namespace,reportScope,processingKey,clone,normalizeEntry,createMemoryStore,validateIdentity,createProcessingStore};
