...
function createProcessingStore({storeFactory=defaultStore,now=()=>Date.now(),randomBytes=crypto.randomBytes,leaseMs=DEFAULT_LEASE_MS}={}){
 ...
 async function acquire({reportId,idempotencyKey,payloadHash},env=process.env){
  const identity=validateIdentity({idempotencyKey,payloadHash}),key=processingKey(reportId,env),store=await storeFactory(),existing=normalizeEntry(await store.getWithMetadata(key,{type:'json'}));
 ...
 async function read(reportId,env=process.env){const key=processingKey(reportId,env),store=await storeFactory();return normalizeEntry(await store.getWithMetadata(key,{type:'json'}))}
