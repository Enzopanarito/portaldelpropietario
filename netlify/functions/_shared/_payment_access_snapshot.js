...
function caracasDay(now=new Date()){const value=new Intl.DateTimeFormat('en-CA',{timeZone:'America/Caracas',day:'2-digit'}).format(now);return Number(value)}
...
function buildAccessSnapshot({owner,expenses=[],payments=[],officialRecords=[],bcvRate,bcvSource='Configuración',now=new Date(),maxAgeMs=DEFAULT_MAX_SNAPSHOT_AGE_MS,month,dueDay=10,surchargeRate=0.10}={}){
 ...
 const rate=assertRate(bcvRate),targetMonth=month||caracasMonth(now),calculationDay=targetMonth===caracasMonth(now)?caracasDay(now):31,official=selectOfficialBalance(officialRecords,house,{month:targetMonth,now,maxAgeMs}),activePayments=relevantActivePayments(owner.id,payments),balance=calculateOwnerBalance(owner,expenses,payments,{now,month:targetMonth,day:calculationDay,dueDay,surchargeRate}),later=official.selected?paymentsAfterCutoff(activePayments,official.selected.cutoffMs):[];
 ...
 const expiredUsd=money(Math.max(0,balance.expiredUsd)),expiredBsRef=money(Math.max(0,balance.expiredBsRef)),requiredBs=money(expiredBsRef*rate),payload={schemaVersion:2,balanceEngineVersion:BALANCE_ENGINE_VERSION,ownerId:owner.id,house,month:targetMonth,dueDay:Number(dueDay),surchargeRate:Number(surchargeRate),capturedAt:now.toISOString(),cutoff:official.selected?.cutoff||now.toISOString(),officialVersion:official.selected?.version||null,officialKey:official.selected?.key||null,source,officialSource:OFFICIAL_SOURCE,cacheValid,invalidReasons,expiredUsd,expiredBsRef,expiredTotalUsd:money(expiredUsd+expiredBsRef),requiredUsdAccount:expiredUsd,requiredBsAccount:requiredBs,bcvRate:rate,bcvSource:clean(bcvSource)||'Configuración',surchargeSnapshot:official.selected?.surchargeBasis||0,paymentWatermark:paymentWatermark(activePayments),paymentsAfterCutoff:later};
...
function validateSnapshotStillCurrent(snapshot,{owner,expenses=[],payments=[],officialRecords=[],bcvRate,bcvSource='Configuración',now=new Date(),maxAgeMs=DEFAULT_MAX_SNAPSHOT_AGE_MS,dueDay=10,surchargeRate=0.10}={}){
 if(!snapshot||snapshot.schemaVersion!==2)return{ok:false,reason:'SNAPSHOT_SCHEMA_INVALID'};
 const current=buildAccessSnapshot({owner,expenses,payments,officialRecords,bcvRate,bcvSource,now,maxAgeMs,month:snapshot.month,dueDay,surchargeRate}),unchanged=current.snapshotId===snapshot.snapshotId;
