...
// Alias de compatibilidad para el endpoint ya validado. Ambos apuntan a campos existentes
// y no son escritos por aggregateFields.
const CONSOLIDATED_FLAG_FIELD = 'Método de Pago';
const AUDIT_OPERATION_FIELD = 'ID de Pago';
...
function paymentMode(payment) {
  return selectName(payment?.fields?.['Forma de Pago'] || 'Bs BCV') === 'USD' ? 'USD' : 'Bs BCV';
}
function isApplied(payment) { return payment?.fields?.['[x] Aplicado al Cierre'] === true; }
function isConsolidated(payment) { return selectName(payment?.fields?.['Método de Pago']) === CONSOLIDATED_METHOD; }
function rollupAmount(payment) { return money(payment?.fields?.['Monto Pagado']); }
...
function amountBs(payment) { return money(payment?.fields?.['Monto Pagado Bs']); }
...
function compareFingerprints(before, after, tolerance = TOLERANCE) {
  const numericFields = ['deudaAnterior', 'deudaAnteriorUsd', 'deudaAnteriorBsRef', 'totalPagado', 'deudaRestante', 'recargoAplicado'];
  const differences = [];
  ...
  for (const field of ['estadoAcceso', 'motivoAcceso']) {
    if (String(before[field] || '') !== String(after[field] || '')) differences.push({ field, before: before[field], after: after[field] });
  }
...
function hashJson(value) { return crypto.createHash('sha256').update(JSON.stringify(canonicalize(value))).digest('hex'); }
...
function buildPlan({ owners = [], payments = [], receipts = [], blockedPaymentIds = new Set(), cutoff, month, retentionDays, snapshotCount = 0 }) {
  ...
  for (const payment of payments) {
    const paymentDate = dateOnly(payment.fields?.['Fecha de Pago']);
    ...
    if (reason) skipped.push({ id: payment.id, fecha: paymentDate, motivo: reason });
    else eligible.push(payment);
  }
  ...
  for (const payment of eligible) {
    ...
    if (!groupsMap.has(key)) groupsMap.set(key, { key, ownerId, mode, payments: [], rollupAmount: 0, equivalentUsd: 0, amountBs: 0, earliestDate: dateOnly(payment.fields?.['Fecha de Pago']) });
  ...
  return {
    generatedAt: new Date().toISOString(), month, cutoff, retentionDays,
    snapshotCount, expectedSnapshotCount, snapshotComplete,
    eligibleCount: eligible.length, skippedCount: skipped.length,
    ownerCount: new Set(eligible.map(paymentOwnerId)).size,
    groupCount: groups.length, totalAmount, totalUsdEquivalent, planHash,
    paymentIds: eligible.map(payment => payment.id), compactPayments, groups, skipped,
    canExecute: eligible.length > 0 && snapshotComplete
  };
}

function aggregateFields(group) {
  const fields = {
...
module.exports = {
  CONSOLIDATED_METHOD, CONSOLIDATED_FLAG_FIELD, AUDIT_OPERATION_FIELD,
  SNAPSHOT_ROWS_PER_OWNER, TOLERANCE,
  money, selectName, dateOnly, paymentOwnerId, paymentMode, isApplied, isConsolidated,
  rollupAmount, equivalentUsd, amountBs, ownerFingerprint, compareFingerprints,
  hashJson, buildPlan, aggregateFields, originalWritableFields
};