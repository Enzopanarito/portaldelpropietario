...
const DEFAULT_PROMPT_VERSION='VLA_PAYMENT_PROOF_V3_2026-08-14';
...
function validateAnalysis(value,{minimumConfidence=0}={}){
 ...
 for(const[name,max]of Object.entries({bank_or_platform:160,reference:160,recipient_name:200,recipient_phone:80,recipient_email:254,recipient_account_visible:120,recipient_account_last4:16,recipient_document:80,recipient_binance_id:120,sender_name:200,sender_account_visible:120,memo:500}))try{nullableString(value[name],max)}catch(error){add('SCHEMA_INVALID',`${name}: ${error.message}`)}
...
function evaluateRawOutput(raw,{minimumConfidence=0.85,maxChars=DEFAULT_MAX_RAW_CHARS}={}){const parsed=parseRawJson(raw,{maxChars});if(!parsed.ok)return parsed;const validation=validateAnalysis(parsed.value,{minimumConfidence});if(!validation.ok){const structurallyValid=!(validation.issueCodes||[]).some(code=>['SCHEMA_INVALID','FORBIDDEN_DECISION_OUTPUT'].includes(code));return{ok:false,reason:failureReason(validation),raw,errors:validation.errors,issueCodes:validation.issueCodes,...(structurallyValid?{normalized:normalizeAnalysis(parsed.value)}:{})}}return{ok:true,raw,normalized:normalizeAnalysis(parsed.value)}}
function safeConfig(config={}){return{aiEnabled:config.aiEnabled===true,primaryModel:clean(config.primaryModel),secondaryModel:clean(config.secondaryModel),primaryTimeoutSeconds:Math.round(boundedNumber(config.primaryTimeoutSeconds,45,10,120)),maximumPrimaryRetries:Math.trunc(boundedNumber(config.maximumPrimaryRetries,0,0,1)),secondaryEnabled:config.secondaryEnabled===true,minimumConfidence:boundedNumber(config.minimumConfidence,0.85,0,1),automaticApprovalEnabled:config.automaticApprovalEnabled===true,minimumAutomaticConfidence:boundedNumber(config.minimumAutomaticConfidence,0.97,0.95,1),externalFallbackEnabled:false,promptVersion:clean(config.promptVersion)||DEFAULT_PROMPT_VERSION}}
