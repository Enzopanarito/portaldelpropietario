...
const OPTIONAL=['recipient_document','recipient_binance_id'];
...
function optionalNullableString(value,maxLength){return value===undefined?null:nullableString(value,maxLength)}
...
function validateAnalysis(value,{minimumConfidence=0}={}){
 ...
 for(const[name,max]of Object.entries({recipient_document:80,recipient_binance_id:120}))if(Object.prototype.hasOwnProperty.call(value,name))try{nullableString(value[name],max)}catch(error){add('SCHEMA_INVALID',`${name}: ${error.message}`)}
...
module.exports={SCHEMA_PATH,DEFAULT_PROMPT_VERSION,DEFAULT_MAX_RAW_CHARS,METHODS,CURRENCIES,STATUSES,REQUIRED,OPTIONAL,ALLOWED,FORBIDDEN_DECISION_KEYS,TRANSIENT_FAILURES,INVALID_OUTPUT_FAILURES,clean,boundedNumber,nullableString,optionalNullableString,normalizeDate,normalizeTime,parseRawJson,validateAnalysis,normalizeAnalysis,failureReason,evaluateRawOutput,safeConfig,nextAiAction,terminalProcessingState,isoOrNull,analysisAudit,schemaManifest};
