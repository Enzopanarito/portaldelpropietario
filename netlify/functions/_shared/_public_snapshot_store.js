...
function validOwner(owner){
 ...
 if(!(Number.isInteger(house)&&house>=1&&house<=EXPECTED_HOUSES&&[usd,bs,total].every(Number.isFinite)&&Math.abs((usd+bs)-total)<=0.011&&owner?.balanceEngineVersion===OWNER_BALANCE_CONTRACT))return false;
 const canonicalUsd=Number(owner.saldoUsd),canonicalBs=Number(owner.saldoBsRef),net=Number(owner.saldoNetoReferencial),payable=Number(owner.totalPagadero),favorUsd=Number(owner.saldoFavorUsd),favorBs=Number(owner.saldoFavorBs),expectedPayable=Math.round((Math.max(0,usd)+Math.max(0,bs)+Number.EPSILON)*100)/100;
 return[canonicalUsd,canonicalBs,net,payable,favorUsd,favorBs,owner.deudaVencidaUsd,owner.deudaVencidaBs,owner.mesCorrienteUsd,owner.mesCorrienteBs].map(Number).every(Number.isFinite)&&Math.abs(canonicalUsd-usd)<=0.011&&Math.abs(canonicalBs-bs)<=0.011&&Math.abs(net-total)<=0.011&&Math.abs(payable-expectedPayable)<=0.011&&Math.abs(favorUsd-Math.max(0,-usd))<=0.011&&Math.abs(favorBs-Math.max(0,-bs))<=0.011;
}
function validatePayload(payload){const errors=[];if(Number(payload&&payload.balanceEngineVersion)!==PUBLIC_DATA_ENGINE_VERSION)errors.push(`La fotografía no usa el motor financiero v${PUBLIC_DATA_ENGINE_VERSION}.`);if(String(payload&&payload.officialBalanceSource||'')!==OFFICIAL_BALANCE_SOURCE)errors.push(`La fotografía no usa ${OFFICIAL_BALANCE_SOURCE}.`);const owners=Array.isArray(payload&&payload.propietarios)?payload.propietarios:[];if(owners.length!==EXPECTED_HOUSES)errors.push(`La fotografía debe contener ${EXPECTED_HOUSES} casas.`);const houses=owners.map(owner=>Number(owner&&owner.Casa));if(new Set(houses).size!==EXPECTED_HOUSES||houses.some((house,index)=>house!==index+1))errors.push('Las casas deben ser únicas y estar ordenadas del 1 al 15.');owners.forEach(owner=>{if(!validOwner(owner))errors.push(`La Casa ${owner&&owner.Casa||'?'} tiene saldos inconsistentes.`)});return{ok:errors.length===0,errors}}
...
function createSnapshotStore({storeFactory=defaultStore,now=()=>Date.now(),config=runtimeConfig}={}){
 // Las lecturas usan el endpoint Blobs disponible en Functions. Las escrituras
 // condicionales por ETag conservan la exclusión y descartan resultados obsoletos.
