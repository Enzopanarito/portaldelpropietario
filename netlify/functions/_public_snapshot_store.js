...
function createMemoryStore(){
 ...
 return{
  ...
  async setJSON(key,data,options={}){
   ...
   entries.set(key,{data:clone(data),etag,metadata:clone(options.metadata||{})});
...
function createSnapshotStore({storeFactory=defaultStore,now=()=>Date.now(),config=runtimeConfig}={}){
 ...
 async function write(payload,env=process.env,expectedEtag){
  if(expectedEtag!==null&&!(typeof expectedEtag==='string'&&expectedEtag.length>0)){const error=new Error('La escritura pública requiere la versión exacta leída previamente.');error.code='PUBLIC_SNAPSHOT_VERSION_REQUIRED';throw error}
  const store=await storeFactory(),snapshot=buildSnapshot(payload,{now:now(),env,config}),options={metadata:{schemaVersion:SCHEMA_VERSION,expiresAt:snapshot.expiresAt,payloadHash:snapshot.payloadHash}};
  if(expectedEtag===null)options.onlyIfNew=true;else options.onlyIfMatch=expectedEtag;
  const result=await store.setJSON(snapshotKey(env,config),snapshot,options);
  if(result.modified===false){const error=new Error('La fotografía cambió durante la reconstrucción; se descartó el resultado antiguo.');error.code='STALE_PUBLIC_SNAPSHOT_WRITE';error.currentEtag=result.etag||'';throw error}
