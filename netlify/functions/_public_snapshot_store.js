...
function enabled(env=process.env,config=runtimeConfig,host=''){
 const normalizedHost=normalizeHost(host),explicit=parseBoolean(env.PUBLIC_BLOB_CACHE_ENABLED);
 if(normalizedHost){if(!isProductionHost(normalizedHost))return false;return explicit===false?false:true}
...
function createMemoryStore(){
 ...
 return{
  ...
  async setJSON(key,data,options={}){const etag=`memory-${++version}`;entries.set(key,{data:clone(data),etag,metadata:clone(options.metadata||{})});return undefined},
...
function buildSnapshot(payload,{now=Date.now(),env=process.env,config=runtimeConfig}={}){
 ...
 return{schemaVersion:SCHEMA_VERSION,cachedAt:new Date(now).toISOString(),expiresAt:now+maxAgeMs(env,config),payloadHash:sha256(JSON.stringify(payload)),payload:clone(payload),invalidated:false,reason:''};
}
...
function createSnapshotStore({storeFactory=defaultStore,now=()=>Date.now(),config=runtimeConfig}={}){
 async function getEntry(key){return normalize(await(await storeFactory()).getWithMetadata(key,{type:'json'}))}
 async function read(env=process.env){
  const entry=await getEntry(snapshotKey(env,config));if(!entry)return{ok:false,reason:'missing'};
  const data=entry.data||{};
  if(data.invalidated)return{ok:false,reason:'invalidated',entry};
 ...
 async function write(payload,env=process.env,expectedEtag){
  const store=await storeFactory(),key=snapshotKey(env,config),before=await getEntry(key);
  if(expectedEtag===null&&before&&!before.data?.invalidated){const error=new Error('La fotografía apareció durante la reconstrucción; se descartó el resultado antiguo.');error.code='STALE_PUBLIC_SNAPSHOT_WRITE';throw error}
  if(typeof expectedEtag==='string'&&expectedEtag&&before?.etag!==expectedEtag){const error=new Error('La fotografía cambió durante la reconstrucción; se descartó el resultado antiguo.');error.code='STALE_PUBLIC_SNAPSHOT_WRITE';throw error}
  const snapshot=buildSnapshot(payload,{now:now(),env,config});
  await store.setJSON(key,snapshot,{metadata:{schemaVersion:SCHEMA_VERSION,expiresAt:snapshot.expiresAt,payloadHash:snapshot.payloadHash}});
  const verified=await getEntry(key);
  if(!verified||verified.data?.payloadHash!==snapshot.payloadHash){const error=new Error('No se pudo verificar la fotografía después de escribirla.');error.code='PUBLIC_SNAPSHOT_WRITE_VERIFY_FAILED';throw error}
  return{ok:true,snapshot,etag:verified.etag||''};
 }
 async function invalidate(reason='financial-write',env=process.env){
  if(!enabled(env,config))return{ok:true,skipped:true};
  const store=await storeFactory(),key=snapshotKey(env,config),tombstone={schemaVersion:SCHEMA_VERSION,invalidated:true,reason:clean(reason).slice(0,120),invalidatedAt:new Date(now()).toISOString(),expiresAt:0,payload:null};
  await store.setJSON(key,tombstone,{metadata:{schemaVersion:SCHEMA_VERSION,invalidated:true,reason:tombstone.reason}});
  const verified=await getEntry(key);return{ok:Boolean(verified&&verified.data?.invalidated===true),etag:verified?.etag||''};
 }
 async function claimRefresh(env=process.env){
  const store=await storeFactory(),key=refreshKey(env,config),timestamp=now(),current=await getEntry(key);
  if(current&&Number(current.data?.expiresAt||0)>timestamp)return{ok:false,reason:'busy'};
  ...
  await store.setJSON(key,lease,{metadata:{expiresAt:lease.expiresAt}});
  const verified=await getEntry(key);
  if(!verified||verified.data?.operationId!==lease.operationId)return{ok:false,reason:'lost'};
  return{ok:true,key,lease,etag:verified.etag||''};
 }
 async function releaseRefresh(marker,env=process.env){
  if(!marker?.ok)return null;
  const store=await storeFactory(),key=marker.key||refreshKey(env,config),current=await getEntry(key);
  ...
  await store.setJSON(key,released,{metadata:{expiresAt:released.expiresAt}});
  return{ok:true};
 }
