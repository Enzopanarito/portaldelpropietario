...
function snapshotIdentity(payload){return`BALANCE_SNAPSHOT_V2|${sha256(payload)}`}
function buildAccessSnapshot({owner,payments=[],officialRecords=[],bcvRate,bcvSource='Configuración',now=new Date(),maxAgeMs=DEFAULT_MAX_SNAPSHOT_AGE_MS,month}={}){
 ...
 return{...payload,snapshotId:snapshotIdentity(payload),automaticEligibility:cacheValid&&invalidReasons.length===0&&balance.expiredTotalRef>TOLERANCE};
}
function validateSnapshotStillCurrent(snapshot,{owner,payments=[],officialRecords=[],bcvRate,bcvSource='Configuración',now=new Date(),maxAgeMs=DEFAULT_MAX_SNAPSHOT_AGE_MS}={}){
 if(!snapshot||snapshot.schemaVersion!==2)return{ok:false,reason:'SNAPSHOT_SCHEMA_INVALID'};
 const current=buildAccessSnapshot({owner,payments,officialRecords,bcvRate,bcvSource,now,maxAgeMs,month:snapshot.month});
 const unchanged=current.paymentWatermark===snapshot.paymentWatermark&&current.expiredUsd===snapshot.expiredUsd&&current.expiredBsRef===snapshot.expiredBsRef&&current.bcvRate===snapshot.bcvRate&&current.officialVersion===snapshot.officialVersion&&current.cutoff===snapshot.cutoff;
