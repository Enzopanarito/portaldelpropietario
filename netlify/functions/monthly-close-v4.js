...
const handler = async function(event) {
  ...
  if (body.action === 'repair') return repairOperation({ month, operationId: String(body.operationId || '').trim(), token: AIRTABLE_API_TOKEN, baseId: AIRTABLE_BASE_ID, counter, json });
  ...
  if (!/^[a-f0-9]{64}$/.test(submittedPlanHash)) return json(400, { success:false, protected:true, message:'La simulación no tiene una huella válida. Vuelva a simular.' }, counter);
  let closeLock = null, handedOff = false;
  try {
    const lockResult = await acquireCloseLock(month, AIRTABLE_API_TOKEN, AIRTABLE_BASE_ID, counter);
    if (!lockResult.ok) return json(409, { success:false, protected:true, closeStatus:lockResult.status, month, repairAvailable:lockResult.status==='partial-error', repairOperationId:lockResult.marker?.operationId||null, message:lockMessage(lockResult,month) }, counter);
    ...
    if (!context.snapshotComplete) {
      await setCloseMarker(closeLock, month, 'ABORTED', AIRTABLE_API_TOKEN, AIRTABLE_BASE_ID, counter).catch(() => null);
      return json(409, { success:false, protected:true, snapshotIncomplete:true, month, snapshotCount:context.snapshotCount, expectedSnapshotCount:context.expectedSnapshotCount, message:`El corte de auditoría está incompleto (${context.snapshotCount}/${context.expectedSnapshotCount}). No se modificó nada.` }, counter);
    }
    handedOff = true;
    return executeClose({ month, closeLock, plan, context, token:AIRTABLE_API_TOKEN, baseId:AIRTABLE_BASE_ID, counter, json });
  } catch (error) {
