(function(){
  ...
  let selectedFile=null,analysisController=null,analyzing=false,manualMode=false,editAll=false,dateEditVisible=false,submissionId='',submitErrorActive=false,analysisData=null,modeUserChosen=false,duplicateReviewPayload=null;
  ...
  function automaticDateFromFile(){return{transactionDate:'',transactionDateSource:'UNDETERMINED',transactionDateConfidence:'LOW',transactionDateNeedsReview:true,transactionDateEvidence:'No se detectó una fecha visible confiable. La fecha de carga y la fecha del archivo no se usan como fecha de pago.'}}
  function dateSourceLabel(source){return({PROOF_EXTRACTED:'leída del comprobante',USER_CONFIRMED:'confirmada por ti · será verificada',UNDETERMINED:'no detectada · requiere revisión'}[source]||'requiere revisión')}
  function displayTransactionDate(value){if(!/^\d{4}-\d{2}-\d{2}$/.test(String(value||'')))return'No detectada';try{return new Intl.DateTimeFormat('es-VE',{timeZone:'America/Caracas',dateStyle:'medium'}).format(new Date(`${value}T12:00:00Z`))}catch(_){return value}}
  ...
  function shortReference(value){const text=String(value||'');return text.length>4?'••••'+text.slice(-4):text||'VLA la verificará'}
  function reportReference(){return byId('payRef')?.value.trim()||''}
  function reportBank(){return byId('payBank')?.value.trim()||''}
  function hideDuplicateChoice(){duplicateReviewPayload=null;byId('vla-pay-duplicate-choice')?.classList.add('hidden')}
  function clearSubmitError(){submitErrorActive=false;hideDuplicateChoice();syncModeFromEvidence();renderProgressiveState();validateForm()}
  ...
  function modalMarkup(){
    return `<div class="vla-pay-sheet modal-card" role="dialog" aria-modal="true" aria-labelledby="vla-pay-title">
      ...
      <form id="reportForm" class="vla-pay-form" novalidate>
        ...
        <div id="vla-pay-duplicate-choice" class="vla-pay-duplicate-choice hidden" role="alertdialog" aria-labelledby="vla-pay-duplicate-title"><b id="vla-pay-duplicate-title">Este comprobante ya fue reportado</b><p id="vla-pay-duplicate-message"></p><div><button id="vla-pay-duplicate-cancel" type="button">Cancelar</button><button id="vla-pay-duplicate-review" type="button">Sí, enviar para revisión</button></div></div>
  ...
  function installMarkup(){
    ...
    byId('closeModal').onclick=hideSmartModal;byId('cancelModal').onclick=hideSmartModal;byId('payProof').addEventListener('change',onFileSelected);byId('vla-pay-manual').onclick=enableManual;byId('vla-pay-edit').onclick=()=>{editAll=true;manualMode=true;dateEditVisible=true;renderProgressiveState();validateForm()};
  ...
  function isNetworkFailure(error){return !Number(error?.status)&&/load failed|failed to fetch|networkerror|network request failed|fetch failed/i.test(String(error?.message||''))}
  function isTransientReportFailure(error){return isNetworkFailure(error)||[502,503,504].includes(Number(error?.status))}
  async function postPaymentReportWithRecovery(payload){
    let lastError=null;
    for(let attempt=0;attempt<2;attempt++){
      try{const response=await fetch('/api/vla/report-payment',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}),data=await response.json().catch(()=>({}));if(!response.ok)throw Object.assign(new Error(data.message||data.detail||'No pudimos completar el reporte.'),{status:response.status,data});return data}
      catch(error){lastError=error;if(attempt===0&&isTransientReportFailure(error)){const submit=byId('submitReport');if(submit)submit.textContent='Reconectando…';await wait(900);continue}break}
    }
    if(isNetworkFailure(lastError))throw Object.assign(new Error('La conexión se interrumpió antes de recibir la confirmación. Es seguro intentarlo nuevamente: el sistema conservará el mismo identificador y no duplicará el reporte.'),{networkFailure:true});
  ...
  function digitalMissing(){
    if(!selectedFile)return['proof'];if(analyzing)return['analysis'];const missing=[];if(!byId('payCurrency').value)missing.push('currency');if(!(enteredAmount()>0))missing.push('amount');syncModeFromEvidence();if(!byId('payMode').value)missing.push('mode');if(!byId('payRef').value.trim())missing.push('reference');if(!byId('payBank').value.trim())missing.push('bank');if((byId('payMode').value==='Bs BCV'||byId('payCurrency').value==='BS')&&!fxRate())missing.push('rate');return missing;
  }
  ...
  function renderConfirmation(){
    const detectedReference=analysisData?.reference||byId('payRef').value.trim(),bank=analysisData?.bank||byId('payBank').value.trim(),recipient=analysisData?.recipient||'No visible',method=methodLabel(analysisData?.method,bank),mode=byId('payMode').value==='Bs BCV'?'Cuenta Bs':'Cuenta USD',currency=byId('payCurrency').value,amount=enteredAmount(),shown=currency==='BS'?realBs(amount):refUsd(amount),status=String(analysisData?.transactionStatus||'').toUpperCase(),date=byId('payTransactionDate').value,dateReview=!date||analysisData?.transactionDateNeedsReview!==false,review=dateReview||(status&&!['COMPLETED','SENT','PROCESSED'].includes(status)),dateLabel=`${displayTransactionDate(date)} · ${dateSourceLabel(analysisData?.transactionDateSource)}`;
    byId('vla-pay-confirm-card').innerHTML=`<strong class="vla-pay-detected-amount">${safeText(shown)} ${safeText(currency==='BS'?'VES':'USD')}</strong><span>${safeText(method)} · ${safeText(bank)}</span><span>${detectedReference?`Confirmación ${safeText(shortReference(detectedReference))}`:'Referencia no detectada'}</span><span>Fecha ${safeText(dateLabel)}</span><span>Receptor ${safeText(recipient)}</span><small>Se aplicará a</small><b>${safeText(mode)}</b>${review?'<p class="vla-pay-review-warning">El comprobante requiere una verificación adicional. Puedes enviarlo; el saldo y el acceso no cambiarán hasta ser validado.</p>':''}`;
  ...
  function switchPaymentChannel(){
    submitErrorActive=false;hideDuplicateChoice();const channel=paymentChannel(),cash=channel==='CASH';if(analysisController)analysisController.abort();analyzing=false;manualMode=cash;editAll=false;dateEditVisible=false;analysisData=null;modeUserChosen=false;byId('reportForm').querySelectorAll('input:not([name="payChannel"]),select,textarea').forEach(node=>{if(node.id!=='payProof')node.value=''});setupModesSmart();
  ...
  function onFileSelected(event){
    submitErrorActive=false;hideDuplicateChoice();const file=event.target.files&&event.target.files[0],label=byId('vla-pay-file-label');selectedFile=file||null;analysisData=null;dateEditVisible=false;modeUserChosen=false;
  ...
  function validateForm(){
    const missing=missingData(),box=byId('vla-pay-validation'),submit=byId('submitReport'),labels={channel:'selecciona cómo pagaste',proof:'adjunta el comprobante',analysis:'espera que termine el análisis',currency:'moneda',amount:'monto',mode:'cuenta correspondiente',reference:'referencia o confirmación',bank:'banco o método',cash:'receptor del efectivo',rate:'tasa BCV disponible'};
    if(!paymentChannel()){box.classList.add('hidden');submit.disabled=true;return missing}box.classList.remove('hidden');
    if(!submitErrorActive){if(missing.length){box.className='vla-pay-validation warn';const visible=paymentChannel()==='DIGITAL'?missing.slice(0,1):missing;box.innerHTML=`<b>${paymentChannel()==='DIGITAL'?'Solo falta un paso':'Necesitamos estos datos'}</b><span>${safeText(visible.map(item=>labels[item]||item).join(' · '))}</span>`}else{box.className='vla-pay-validation ok';box.innerHTML='<b>Listo para confirmar</b><span>VLA volverá a verificar el comprobante antes de aplicar cualquier cambio.</span>'}}
    submit.disabled=missing.length>0;return missing;
  }
  function openSmartReport(){
    if(typeof currentOwner==='undefined'||!currentOwner)return;installMarkup();selectedFile=null;submissionId=newSubmissionId();manualMode=false;editAll=false;dateEditVisible=false;analyzing=false;submitErrorActive=false;analysisData=null;modeUserChosen=false;duplicateReviewPayload=null;if(analysisController)analysisController.abort();byId('reportForm').reset();byId('payChannelDigital').checked=true;byId('vla-pay-file-label').textContent='Tomar foto / Elegir comprobante';byId('vla-pay-notes-count').textContent='0';byId('vla-pay-manual').disabled=true;setupModesSmart();switchPaymentChannel();const modal=byId('modal');modal.classList.remove('hidden');modal.classList.add('flex');document.documentElement.classList.add('vla-pay-open');byId('payProof').focus();
  }
  function hideSmartModal(){const modal=byId('modal');if(!modal)return;if(analysisController)analysisController.abort();modal.classList.add('hidden');modal.classList.remove('flex');document.documentElement.classList.remove('vla-pay-open');selectedFile=null;analyzing=false;manualMode=false;editAll=false;dateEditVisible=false;analysisData=null;submitErrorActive=false;modeUserChosen=false;duplicateReviewPayload=null}
  async function submitSmartReport(event){
    event.preventDefault();submitErrorActive=false;const missing=validateForm();if(missing.length)return;const submit=byId('submitReport');let payload=null;submit.disabled=true;submit.textContent='Enviando…';
    try{
      const amount=enteredAmount(),enteredCurrency=byId('payCurrency').value,mode=byId('payMode').value,channel=paymentChannel(),attachment=channel==='DIGITAL'?await fileToPayload(selectedFile):null,automatic=automaticDateFromFile(),transactionDate=channel==='DIGITAL'?byId('payTransactionDate').value:'',dateSource=analysisData?.transactionDateSource||(channel==='DIGITAL'?automatic.transactionDateSource:'');
      payload={ownerId:currentOwner.id,submissionId,mode,amount,enteredCurrency,paymentChannel:channel,reference:channel==='DIGITAL'?reportReference():'',rate:fxRate(),bank:channel==='DIGITAL'?reportBank():'Efectivo',method:channel==='DIGITAL'?(analysisData?.method||'OTHER'):'CASH',cashReceiver:channel==='CASH'?byId('payCashReceiver').value.trim():'',transactionDate,transactionDateSource:dateSource,dateAttestation:channel==='DIGITAL'?(analysisData?.dateAttestation||''):'',transactionDateConfidence:analysisData?.transactionDateConfidence||automatic.transactionDateConfidence,transactionDateNeedsReview:analysisData?.transactionDateNeedsReview!==false,transactionDateEvidence:analysisData?.transactionDateEvidence||automatic.transactionDateEvidence,analysisSummary:channel==='DIGITAL'?{provider:analysisData?.analysisProvider||'',route:analysisData?.analysisRoute||'',confidence:number(analysisData?.confidence),transactionTime:analysisData?.transactionTime||'',transactionStatus:analysisData?.transactionStatus||'UNKNOWN',recipient:analysisData?.recipient||'',warnings:Array.isArray(analysisData?.warnings)?analysisData.warnings.slice(0,5):[],possibleVisualModification:analysisData?.possibleVisualModification===true,prefillComplete:analysisData?.prefillComplete===true,missingLabels:Array.isArray(analysisData?.missingLabels)?analysisData.missingLabels.slice(0,8):[],dateSource,dateConfidence:analysisData?.transactionDateConfidence||automatic.transactionDateConfidence,dateNeedsReview:analysisData?.transactionDateNeedsReview!==false,dateEvidence:analysisData?.transactionDateEvidence||automatic.transactionDateEvidence}:null,observations:byId('payNotes').value.trim(),attachment};
      const data=await postPaymentReportWithRecovery(payload);hideSmartModal();if(typeof toast==='function')toast(data.message||'Reporte recibido.',false);
    }catch(error){const box=byId('vla-pay-validation'),duplicate=Number(error.status)===409||error.data?.duplicate===true,message=error.message||'Revise los datos e intente nuevamente.';submitErrorActive=true;box.className='vla-pay-validation warn';box.innerHTML=`<b>${duplicate?'Este comprobante ya fue reportado':'No se envió el reporte'}</b><span>${safeText(message)}</span>`;if(duplicate&&error.data?.canSubmitForReview===true&&payload){duplicateReviewPayload=payload;byId('vla-pay-duplicate-message').textContent=message;byId('vla-pay-duplicate-choice').classList.remove('hidden')}else if(typeof toast==='function')toast(message,true)}finally{submit.textContent=paymentChannel()==='CASH'?'Reportar efectivo':'Confirmar pago';if(!byId('modal').classList.contains('hidden'))validateForm()}
  }
  function cancelDuplicateReview(){duplicateReviewPayload=null;byId('vla-pay-duplicate-choice').classList.add('hidden');const box=byId('vla-pay-validation');submitErrorActive=true;box.className='vla-pay-validation warn';box.innerHTML='<b>No se creó ningún reporte</b><span>Puedes elegir otro comprobante o cerrar esta ventana.</span>'}
  async function submitDuplicateReview(){if(!duplicateReviewPayload||window.vlaDuplicateReviewBusy)return;const button=byId('vla-pay-duplicate-review');try{window.vlaDuplicateReviewBusy=true;button.disabled=true;button.textContent='Enviando…';const data=await postPaymentReportWithRecovery({...duplicateReviewPayload,duplicateReviewRequested:true});hideSmartModal();if(typeof toast==='function')toast(data.message||'Reporte recibido para revisión.',false)}catch(error){const box=byId('vla-pay-validation'),message=error.message||'No se pudo enviar la revisión especial.';submitErrorActive=true;box.className='vla-pay-validation warn';box.innerHTML=`<b>No se envió el reporte</b><span>${safeText(message)}</span>`;if(typeof toast==='function')toast(message,true)}finally{window.vlaDuplicateReviewBusy=false;button.disabled=false;button.textContent='Sí, enviar para revisión'}}
  ...
  function install(){if(!window.VLAPaymentIntelligence||!byId('modal'))return setTimeout(install,30);installMarkup();bindButtons();try{openReport=openSmartReport;hideModal=hideSmartModal;setupModes=setupModesSmart}catch(_){}document.documentElement.dataset.vlaOwnerPaymentReport='progressive-v10'}
