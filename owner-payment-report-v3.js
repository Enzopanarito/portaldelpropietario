(function(){
  ...
  function fillFromAnalysis(data){
    analysisData={...(data.analysis||{}),analysisProvider:data.analysisProvider||'',analysisRoute:data.analysisRoute||'',prefillComplete:data.complete===true,missingLabels:(data.missing||[]).map(item=>item.label)};const a=analysisData;byId('payCurrency').value=a.currency==='VES'?'BS':a.currency==='USD'?'USD':'';byId('payAmount').value=a.amount?String(a.amount).replace('.',','):'';byId('payBank').value=a.bank||'';byId('payRef').value=a.reference||'';byId('payTransactionDate').value=a.transactionDate||automaticDateFromFile(selectedFile).transactionDate;byId('payTransactionStatus').value=a.transactionStatus||'';syncModeFromCurrency();
