(function(){
  ...
  let selectedFile=null,analysisController=null,analyzing=false,manualMode=false,editAll=false,dateEditVisible=false,submissionId='',submitErrorActive=false,analysisData=null;
  ...
  function serverDateMethod(method){return['ZELLE','BINANCE_PAY','CRYPTO_TRANSFER'].includes(String(method||'').toUpperCase())}
  function normalizedMethod(method,bank=''){const explicit=String(method||'').toUpperCase(),hint=String(bank||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toUpperCase();if(hint.includes('ZELLE'))return'ZELLE';if(hint.includes('BINANCE PAY'))return'BINANCE_PAY';if(hint.includes('BINANCE')||hint.includes('CRIPTO')||hint.includes('CRYPTO'))return'CRYPTO_TRANSFER';return explicit||'OTHER'}
  function automaticDateFromFile(file,method=''){const now=new Date();if(serverDateMethod(method))return{transactionDate:caracasDateISO(now),transactionDateSource:'REPORT_TIMESTAMP_FALLBACK',transactionDateConfidence:'LOW',transactionDateNeedsReview:false,transactionDateEvidence:'Sin fecha visible confiable; se usará automáticamente la fecha/hora oficial de recepción del servidor en Venezuela.'};const modified=Number(file?.lastModified||0),oldest=new Date(now);oldest.setUTCFullYear(oldest.getUTCFullYear()-2);if(modified&&new Date(modified)>=oldest&&modified<=now.getTime()+86400000)return{transactionDate:caracasDateISO(new Date(modified)),transactionDateSource:'FILE_LAST_MODIFIED',transactionDateConfidence:'MEDIUM',transactionDateNeedsReview:true,transactionDateEvidence:'Fecha estimada desde los metadatos del archivo.'};return{transactionDate:caracasDateISO(now),transactionDateSource:'REPORT_TIMESTAMP_FALLBACK',transactionDateConfidence:'LOW',transactionDateNeedsReview:true,transactionDateEvidence:'Fecha oficial de Venezuela al momento del reporte.'}}
  function dateSourceLabel(source){return({PROOF_EXTRACTED:'leída del comprobante',FILE_LAST_MODIFIED:'estimada desde la fecha del archivo · será verificada',REPORT_TIMESTAMP_FALLBACK:'fecha oficial del reporte · será verificada',USER_CONFIRMED:'confirmada por ti · será verificada'}[source]||'será verificada')}
  function displayTransactionDate(value){if(!/^\d{4}-\d{2}-\d{2}$/.test(String(value||'')))return'Se calculará al enviar';try{return new Intl.DateTimeFormat('es-VE',{timeZone:'America/Caracas',dateStyle:'medium'}).format(new Date(`${value}T12:00:00Z`))}catch(_){return value}}
  ...
  function installMarkup(){
    ...
    byId('closeModal').onclick=hideSmartModal;byId('cancelModal').onclick=hideSmartModal;byId('payProof').addEventListener('change',onFileSelected);byId('vla-pay-manual').onclick=enableManual;byId('vla-pay-edit').onclick=()=>{editAll=true;dateEditVisible=true;manualMode=true;renderProgressiveState();validateForm()};
    ...
    ['payAmount','payBank','payCashReceiver','payRef'].forEach(id=>byId(id).addEventListener('input',clearSubmitError));
  ...
  function syncModeFromCurrency(){const mode=byId('payMode'),currency=byId('payCurrency')?.value,cash=paymentChannel()==='CASH',suggested=cash?(currency==='BS'?'Bs BCV':currency==='USD'?'USD':''):suggestedMode(currency);if(mode&&!mode.value&&suggested)mode.value=suggested}
  ...
  function digitalMissing(){
    if(!selectedFile)return['proof'];if(analyzing)return['analysis'];const missing=[];if(!byId('payCurrency').value)missing.push('currency');if(!(enteredAmount()>0))missing.push('amount');if(!byId('payMode').value)missing.push('mode');if(!byId('payBank').value.trim()&&!analysisData?.method)missing.push('bank');if(!byId('payRef').value.trim())missing.push('reference');if((byId('payMode').value==='Bs BCV'||byId('payCurrency').value==='BS')&&!fxRate())missing.push('rate');return missing;
  }
  ...
  function renderProgressiveState(){
    ...
    Object.entries(FIELD_IDS).forEach(([key,id])=>{let show=editAll||manualMode||missing.includes(key);if(key==='date')show=dateEditVisible;if(cash)show=['currency','amount','cash','notes'].includes(key);if(!cash&&key==='cash')show=false;byId(id).classList.toggle('hidden',!show)});
  ...
  function fillFromAnalysis(data){
    analysisData={...(data.analysis||{}),analysisProvider:data.analysisProvider||'',analysisRoute:data.analysisRoute||'',prefillComplete:data.complete===true,missingLabels:(data.missing||[]).map(item=>item.label)};const a=analysisData;byId('payCurrency').value=a.currency==='VES'?'BS':a.currency==='USD'?'USD':'';byId('payAmount').value=a.amount?String(a.amount).replace('.',','):'';byId('payBank').value=a.bank||'';byId('payRef').value=a.reference||'';byId('payTransactionDate').value=a.transactionDate||automaticDateFromFile(selectedFile,a.method).transactionDate;byId('payTransactionStatus').value=a.transactionStatus||'';syncModeFromCurrency();
    const confidence=Math.round(number(a.confidence)*100),missing=(data.missing||[]).map(item=>item.label).join(', '),dateText=a.transactionDateNeedsReview?' La fecha se estimó automáticamente y quedará sujeta a verificación.':'';scanMessage(data.complete?'ok':'warn',data.complete?'Listo':'Necesitamos un dato adicional',data.complete?`Comprobante leído para confirmación · confianza ${confidence}%.${dateText}`:`Completa solamente: ${missing||'el dato señalado'}.${dateText}`);renderProgressiveState();
  }
  function switchPaymentChannel(){
    submitErrorActive=false;const channel=paymentChannel(),cash=channel==='CASH';if(analysisController)analysisController.abort();analyzing=false;manualMode=cash;editAll=false;dateEditVisible=false;analysisData=null;byId('reportForm').querySelectorAll('input:not([name="payChannel"]),select,textarea').forEach(node=>{if(node.id!=='payProof')node.value=''});setupModesSmart();
  ...
  async function analyzeProof(){
    ...
    try{const attachment=await fileToPayload(selectedFile),response=await fetch('/api/vla/payment-proof-prefill',{method:'POST',headers:{'Content-Type':'application/json'},signal:analysisController.signal,body:JSON.stringify({ownerId:currentOwner.id,attachment})}),data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.message||'No pudimos leer todo automáticamente.');scanMessage('loading','Verificando datos','Aplicando las reglas del método y la cuenta correspondiente.');fillFromAnalysis(data)}catch(error){if(error.name==='AbortError')return;const date=automaticDateFromFile(selectedFile);byId('payTransactionDate').value=date.transactionDate;scanMessage('warn','La lectura necesita apoyo',`${error.message||'Completa los datos faltantes.'} La fecha ya quedó estimada automáticamente.`);manualMode=true;analysisData={method:'OTHER',transactionStatus:'UNKNOWN',confidence:0,warnings:['La prelectura no estuvo disponible.'],...date}}finally{analyzing=false;renderProgressiveState();validateForm()}
  }
  function enableManual(){if(!selectedFile)return;manualMode=true;editAll=true;dateEditVisible=false;if(analysisController)analysisController.abort();analyzing=false;scanMessage('manual','Carga manual activa','Completa los datos principales; la fecha ya fue resuelta automáticamente y el backend volverá a validarla.');renderProgressiveState();validateForm();byId('payCurrency').focus()}
  function onFileSelected(event){
    submitErrorActive=false;const file=event.target.files&&event.target.files[0],label=byId('vla-pay-file-label');selectedFile=file||null;analysisData=null;dateEditVisible=false;
  ...
  async function submitSmartReport(event){
    ...
    try{const amount=enteredAmount(),enteredCurrency=byId('payCurrency').value,mode=byId('payMode').value,channel=paymentChannel(),attachment=channel==='DIGITAL'?await fileToPayload(selectedFile):null,bank=channel==='DIGITAL'?byId('payBank').value.trim():'Efectivo',method=channel==='DIGITAL'?normalizedMethod(analysisData?.method||'OTHER',bank):'CASH',automatic=automaticDateFromFile(selectedFile,method),trustedVisible=analysisData?.transactionDateSource==='PROOF_EXTRACTED'&&Boolean(analysisData?.transactionDateAttestation),transactionDate=channel==='DIGITAL'?(trustedVisible?(byId('payTransactionDate').value||automatic.transactionDate):serverDateMethod(method)?automatic.transactionDate:(byId('payTransactionDate').value||automatic.transactionDate)):'',dateSource=trustedVisible?'PROOF_EXTRACTED':serverDateMethod(method)?automatic.transactionDateSource:(analysisData?.transactionDateSource||automatic.transactionDateSource),payload={ownerId:currentOwner.id,submissionId,mode,amount,enteredCurrency,paymentChannel:channel,reference:byId('payRef').value.trim(),rate:fxRate(),bank,method,cashReceiver:channel==='CASH'?byId('payCashReceiver').value.trim():'',transactionDate,transactionDateSource:dateSource,transactionDateAttestation:trustedVisible?analysisData.transactionDateAttestation:'',transactionDateConfidence:trustedVisible?analysisData.transactionDateConfidence:automatic.transactionDateConfidence,transactionDateNeedsReview:trustedVisible?analysisData.transactionDateNeedsReview!==false:automatic.transactionDateNeedsReview!==false,transactionDateEvidence:trustedVisible?analysisData.transactionDateEvidence:automatic.transactionDateEvidence,analysisSummary:channel==='DIGITAL'?{provider:analysisData?.analysisProvider||'',route:analysisData?.analysisRoute||'',confidence:number(analysisData?.confidence),transactionTime:analysisData?.transactionTime||'',transactionStatus:analysisData?.transactionStatus||'UNKNOWN',recipient:analysisData?.recipient||'',warnings:Array.isArray(analysisData?.warnings)?analysisData.warnings.slice(0,5):[],possibleVisualModification:analysisData?.possibleVisualModification===true,prefillComplete:analysisData?.prefillComplete===true,missingLabels:Array.isArray(analysisData?.missingLabels)?analysisData.missingLabels.slice(0,8):[],dateSource,dateConfidence:trustedVisible?analysisData.transactionDateConfidence:automatic.transactionDateConfidence,dateNeedsReview:trustedVisible?analysisData.transactionDateNeedsReview!==false:automatic.transactionDateNeedsReview!==false,dateEvidence:trustedVisible?analysisData.transactionDateEvidence:automatic.transactionDateEvidence}:null,observations:byId('payNotes').value.trim(),attachment},data=await postPaymentReportWithRecovery(payload);hideSmartModal();if(typeof toast==='function')toast(data.message||'Reporte recibido.',false)}catch(error){const box=byId('vla-pay-validation'),duplicate=Number(error.status)===409||error.data?.duplicate===true,message=error.message||'Revise los datos e intente nuevamente.';submitErrorActive=true;box.className='vla-pay-validation warn';box.innerHTML=`<b>${duplicate?'Este comprobante ya fue reportado':'No se envió el reporte'}</b><span>${safeText(message)}</span>`;if(typeof toast==='function')toast(message,true)}finally{submit.textContent=paymentChannel()==='CASH'?'Reportar efectivo':'Confirmar pago';if(!byId('modal').classList.contains('hidden'))validateForm()}
  }
  ...
  function install(){if(!window.VLAPaymentIntelligence||!byId('modal'))return setTimeout(install,30);installMarkup();bindButtons();try{openReport=openSmartReport;hideModal=hideSmartModal;setupModes=setupModesSmart}catch(_){}document.documentElement.dataset.vlaOwnerPaymentReport='progressive-v8'}
