(function(){
  ...
  let selectedFile=null,analysisController=null,analyzing=false,manualMode=false,editAll=false,dateEditVisible=false,submissionId='',submitErrorActive=false,analysisData=null,modeUserChosen=false;
  ...
  function generatedReference(){return`PENDIENTE-${String(submissionId||newSubmissionId()).replace(/[^A-Za-z0-9]/g,'').slice(-12)}`}
  function reportReference(){const entered=byId('payRef')?.value.trim()||'';return entered||generatedReference()}
  function reportBank(){const entered=byId('payBank')?.value.trim()||'';if(entered)return entered;const detected=methodLabel(analysisData?.method,'');return detected==='Método por verificar'?'Verificación automática VLA':detected}
  function clearSubmitError(){submitErrorActive=false;syncModeFromEvidence();renderProgressiveState();validateForm()}
  ...
  function digitalMissing(){
    if(!selectedFile)return['proof'];if(analyzing)return['analysis'];const missing=[];if(!byId('payCurrency').value)missing.push('currency');if(!(enteredAmount()>0))missing.push('amount');syncModeFromEvidence();if(!byId('payMode').value)missing.push('mode');if((byId('payMode').value==='Bs BCV'||byId('payCurrency').value==='BS')&&!fxRate())missing.push('rate');return missing;
  }
  ...
  function renderConfirmation(){
    const detectedReference=analysisData?.reference||byId('payRef').value.trim(),method=methodLabel(analysisData?.method,analysisData?.bank||byId('payBank').value.trim()),mode=byId('payMode').value==='Bs BCV'?'Cuenta Bs':'Cuenta USD',currency=byId('payCurrency').value,amount=enteredAmount(),shown=currency==='BS'?realBs(amount):refUsd(amount),status=String(analysisData?.transactionStatus||'').toUpperCase(),review=status&&!['COMPLETED','SENT','PROCESSED'].includes(status),date=byId('payTransactionDate').value,dateLabel=`${displayTransactionDate(date)} · ${dateSourceLabel(analysisData?.transactionDateSource)}`;
    byId('vla-pay-confirm-card').innerHTML=`<strong class="vla-pay-detected-amount">${safeText(shown)} ${safeText(currency==='BS'?'VES':'USD')}</strong><span>${safeText(method)}</span><span>${detectedReference?`Confirmación ${safeText(shortReference(detectedReference))}`:'Referencia: VLA la verificará'}</span><span>Fecha ${safeText(dateLabel)}</span><small>Se aplicará a</small><b>${safeText(mode)}</b>${review?'<p class="vla-pay-review-warning">El comprobante requiere una verificación adicional. Puedes enviarlo; el saldo no cambiará hasta ser validado.</p>':''}`;
  ...
  function fillFromAnalysis(data){
    analysisData={...(data.analysis||{}),analysisProvider:data.analysisProvider||'',analysisRoute:data.analysisRoute||'',prefillComplete:data.complete===true,missingLabels:(data.missing||[]).map(item=>item.label)};const a=analysisData;byId('payCurrency').value=a.currency==='VES'?'BS':a.currency==='USD'?'USD':'';byId('payAmount').value=a.amount?String(a.amount).replace('.',','):'';byId('payBank').value=a.bank||'';byId('payRef').value=a.reference||'';byId('payTransactionDate').value=a.transactionDate||automaticDateFromFile(selectedFile).transactionDate;byId('payTransactionStatus').value=a.transactionStatus||'';modeUserChosen=false;syncModeFromEvidence();
    const missing=digitalMissing().filter(item=>!['proof','analysis','rate'].includes(item)),serverMissing=(data.missing||[]).map(item=>item.label);if(!missing.length){const verifyLater=serverMissing.length?` VLA volverá a verificar ${serverMissing.join(' y ')} en segundo plano.`:'';scanMessage('ok','Comprobante leído',`Revisa el resumen y confirma.${verifyLater}`)}else{const next={currency:'la moneda',amount:'el monto',mode:'a cuál saldo corresponde'}[missing[0]]||'el dato señalado';scanMessage('warn','Casi listo',`Solo falta confirmar ${next}. VLA verificará el resto automáticamente.`)}renderProgressiveState();
  }
  function switchPaymentChannel(){
    submitErrorActive=false;const channel=paymentChannel(),cash=channel==='CASH';if(analysisController)analysisController.abort();analyzing=false;manualMode=cash;editAll=false;dateEditVisible=false;analysisData=null;modeUserChosen=false;byId('reportForm').querySelectorAll('input:not([name="payChannel"]),select,textarea').forEach(node=>{if(node.id!=='payProof')node.value=''});setupModesSmart();
  ...
  function onFileSelected(event){
    submitErrorActive=false;const file=event.target.files&&event.target.files[0],label=byId('vla-pay-file-label');selectedFile=file||null;analysisData=null;dateEditVisible=false;modeUserChosen=false;
  ...
  function validateForm(){
    const missing=missingData(),box=byId('vla-pay-validation'),submit=byId('submitReport'),labels={channel:'selecciona cómo pagaste',proof:'adjunta el comprobante',analysis:'espera que termine el análisis',currency:'moneda',amount:'monto',mode:'cuenta correspondiente',cash:'receptor del efectivo',rate:'tasa BCV disponible'};
