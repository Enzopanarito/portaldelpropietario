...
const OUTPUT=path.join(__dirname,'..','netlify','functions','_runtime_config.generated.js');
...
function write(config=buildConfig(),output=OUTPUT){fs.mkdirSync(path.dirname(output),{recursive:true});fs.writeFileSync(output,render(config),'utf8');return output}
...
module.exports={OUTPUT,ALLOWED_CONTEXTS,ALLOWED_DATA_ENVIRONMENTS,clean,parseBoolean,boundedInteger,buildConfig,render,write};
