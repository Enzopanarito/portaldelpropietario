...
const ves=value=>`Bs. ${num(value).toLocaleString('es-VE',{minimumFractionDigits:2,maximumFractionDigits:2})}`;
...
function ensurePanel(){
 ...
 host.insertAdjacentHTML('beforeend',`<section id="vla-autopay-supervision" class="vla-autopay-panel"><div class="vla-autopay-heading"><div><h3>Historial de pagos validados</h3><p>Incluye pagos aprobados por el administrador y pagos autovalidados. Las reversiones conservan el reporte y el comprobante para auditoría.</p></div><button id="vla-autopay-refresh" type="button">Actualizar historial</button></div><div id="vla-autopay-summary" class="vla-autopay-summary"></div><div id="vla-autopay-body" class="vla-autopay-body"><div class="vla-autopay-empty">Abre Pagos para cargar el historial de validaciones.</div></div></section>`);
...
function summaryHtml(summary={}){
 return `<div><span>Activos</span><b>${num(summary.active)}</b></div><div><span>Administrador</span><b>${num(summary.manual)}</b></div><div><span>Automáticos</span><b>${num(summary.automatic)}</b></div><div><span>Revertidos</span><b>${num(summary.reverted)}</b></div><div><span>Total activo</span><b>${usd(summary.totalActiveUsd)}</b></div>`;
}
function statusBadge(item){
 if(item.status==='REVERTIDO')return'<span class="vla-autopay-status reverted">Revertido</span>';
...
function validationDetail(item){
 if(item.validationSource==='AUTOMATIC'){
   const receiver=item.receiver?`Receptor ${item.receiver}`:'Motor automático';
   return {title:'Autovalidado',detail:`${receiver}${item.confidence?` · IA ${pct(item.confidence)}`:''}`};
 }
 return {title:'Administrador',detail:item.reviewedBy||item.decision||'Validación manual'};
}
function card(item){
 const method=[item.method,item.platform].filter(Boolean).join(' · ')||'—';
 const validation=validationDetail(item);
 ...
 const reverseButton=item.canReverse?`<button class="danger" data-autopay-action="reverse" data-report-id="${esc(item.reportId)}" data-payment-id="${esc(item.paymentId)}">Revertir aprobación</button>`:'';
 return `<article class="vla-autopay-card"><div class="vla-autopay-card-head"><div><strong>Casa ${esc(item.house||'—')} · ${esc(item.ownerName||'')}</strong><small>${esc(item.validationLabel||'Pago validado')} · ${esc(date(item.approvedAt))}</small></div>${statusBadge(item)}</div><div class="vla-autopay-grid"><div><span>Monto aplicado</span><b>${usd(item.amountUsd)}</b>${item.amountBs?`<small>Bs aplicados: ${ves(item.amountBs)}</small>`:''}${item.receivedCurrency==='VES'&&item.receivedAmount?`<small>Reportado: ${ves(item.receivedAmount)}</small>`:''}</div><div><span>Referencia</span><b>${esc(item.reference||'—')}</b><small>${esc(item.paymentDate||'')}</small></div><div><span>Método</span><b>${esc(method)}</b><small>${esc(item.mode||'')}</small></div><div><span>Validación</span><b>${esc(validation.title)}</b><small>${esc(validation.detail)}</small></div></div>${closeNotice}${reversal}<div class="vla-autopay-actions"><button data-autopay-action="proof" data-report-id="${esc(item.reportId)}">Ver comprobante</button>${reverseButton}</div></article>`;
}
function render(data){
 ...
 body.innerHTML=items.length?items.map(card).join(''):'<div class="vla-autopay-empty">Todavía no hay pagos validados para mostrar.</div>';
}
async function loadHistory(force=false){
 ensurePanel();if(loading)return;if(!force&&Date.now()-lastLoadedAt<30000)return;
 const body=document.getElementById('vla-autopay-body');loading=true;if(body)body.innerHTML='<div class="vla-autopay-empty">Cargando historial de pagos…</div>';
 try{
...
async function reversePayment(button){
 ...
 const reason=text(prompt('Motivo obligatorio de la reversión:')||'').trim();
 if(!reason)return;
 if(reason.length<10){if(typeof toast==='function')toast('Explica el motivo con al menos 10 caracteres.',true);return;}
 if(!confirm('Esta acción retirará el pago definitivo, recalculará el saldo y volverá a evaluar el acceso de la propiedad. El reporte y el comprobante seguirán guardados para auditoría. ¿Confirmas la reversión?'))return;
 ...
 try{
   ...
   if(typeof toast==='function')toast(result.message||'Pago revertido.');
...
function install(){
 ...
 document.documentElement.dataset.vlaAutopaySupervision='v2';
}
